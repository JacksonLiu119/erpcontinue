...
// 《iSM-自動分錄系統》AJSI01 的設定以既有 accounting_system_parameters 保存，
// 不另建表；AJSI02～AJSI24 的文件節點則用這份目錄對照既有單據／分錄規則。
// 這些是目標 ERP 的預設值，會依目前登入的公司／資料來源建立，絕不寫入 SH／SC 原始庫。
const autoJournalSettingDefaults = [
  { code:'AUT_AJSI01_ACCOUNTING_CLASSIFICATION', name:'會計分類方式', value:'1', dataType:'text', note:'1～4；依文件控制庫存／銷售／銷退科目取值' },
  { code:'AUT_AJSI01_POSTING_METHOD', name:'拋轉方式', value:'draft_only', dataType:'text', note:'draft_only＝拋轉自動分錄底稿；draft_and_voucher＝底稿及會計傳票' },
  { code:'AUT_AJSI01_SAME_ACCOUNT_AGGREGATION', name:'同底稿科目彙總', value:'1', dataType:'boolean', note:'同一底稿的相同科目是否彙總成一筆' },
  { code:'AUT_AJSI01_DRAFT_OPENING_METHOD', name:'底稿產生方式', value:'aggregate', dataType:'text', note:'aggregate＝彙總；per_document＝逐張' },
  { code:'AUT_AJSI01_DEPARTMENT_CONTROL', name:'部門控制', value:'0', dataType:'boolean', note:'啟用後分錄來源需帶入目前授權部門' },
  { code:'AUT_AJSI01_DEBIT_SUMMARY_SOURCE', name:'借方摘要來源', value:'document_no', dataType:'text', note:'單別單號／異動日期／備註／部門名稱' },
  { code:'AUT_AJSI01_CREDIT_SUMMARY_SOURCE', name:'貸方摘要來源', value:'document_no', dataType:'text', note:'單別單號／異動日期／備註／部門名稱' },
  { code:'AUT_AJSI01_TRANSFER_JOURNAL_TYPE', name:'轉帳傳票單別', value:'91', dataType:'text', note:'文件預設 91 轉帳傳票' },
  { code:'AUT_AJSI01_TRANSFER_JOURNAL_NATURE', name:'轉帳傳票性質', value:'general', dataType:'text', note:'文件預設一般' }
];

const autoJournalNatureCatalog = [
  ['AJSI02','分錄性質設定-庫存異動單','INV','inventory_movement','main','庫存異動單據'],
  ['AJSI03','分錄性質設定-銷貨單','AR','shipment','main','銷貨單'],
  ['AJSI04','分錄性質設定-銷退單','AR','sales_return','main','銷退單'],
  ['AJSI05','分錄性質設定-銷貨成本','AR','shipment','cost','銷貨成本'],
  ['AJSI06','分錄性質設定-銷退成本','AR','sales_return','cost','銷退成本'],
  ['AJSI07','分錄性質設定-結帳單','AR','settlement','main','結帳單'],
  ['AJSI08','分錄性質設定-收款單','AR','settlement','main','收款單'],
  ['AJSI09','分錄性質設定-進貨單','AP','purchase_receipt','main','進貨單'],
  ['AJSI10','分錄性質設定-退貨單','AP','purchase_return','main','退貨單'],
  ['AJSI11','分錄性質設定-應付憑單','AP','purchase_receipt','main','應付憑單'],
  ['AJSI12','分錄性質設定-付款單','AP','settlement','main','付款單'],
  ['AJSI13','分錄性質設定-領料單','MOC','material_issue','main','領料單'],
  ['AJSI14','分錄性質設定-退料單','MOC','material_return','main','退料單'],
  ['AJSI15','分錄性質設定-生產入庫單','MOC','production_receipt','main','生產入庫單'],
  ['AJSI16','分錄性質設定-託外進貨費用','MOC','outsourcing_receipt','expense','託外進貨費用'],
  ['AJSI17','分錄性質設定-託外進貨成本','MOC','outsourcing_receipt','cost','託外進貨成本'],
  ['AJSI18','分錄性質設定-託外退費用','MOC','outsourcing_return','expense','託外退費用'],
  ['AJSI19','分錄性質設定-託外退成本','MOC','outsourcing_return','cost','託外退成本'],
  ['AJSI24','分錄性質設定-成本開帳/調整單','INV','cost_opening_adjustment','main','成本開帳／調整單']
].map(([code,label,moduleCode,documentKind,entryRole,sourceLabel]) => ({ code,label,module_code:moduleCode,document_kind:documentKind,entry_role:entryRole,source_label:sourceLabel }));
...
function registerAccountingWorkflowRoutes(app) {
  const accounts = [
    ['1001','銀行存款','asset'],['1101','應收帳款','asset'],['1121','應收票據','asset'],['1122','應收票據託收','asset'],['1131','預付貨款','asset'],['1201','商品存貨','asset'],['1501','累計折舊','asset'],['2101','應付帳款','liability'],['2141','應付票據','liability'],['2201','客戶預收款','liability'],
  ...
  const contextParams = c => [c.tenant_id,c.company_id,c.source_system,c.source_database];
  ...
  const autoJournalSettingsByCode = new Map(autoJournalSettingDefaults.map(row => [row.code,row]));
  const autoJournalSettingAliases = {
    accounting_classification:'AUT_AJSI01_ACCOUNTING_CLASSIFICATION',
    posting_method:'AUT_AJSI01_POSTING_METHOD',
    same_account_aggregation:'AUT_AJSI01_SAME_ACCOUNT_AGGREGATION',
    aggregate_accounts:'AUT_AJSI01_SAME_ACCOUNT_AGGREGATION',
    draft_opening_method:'AUT_AJSI01_DRAFT_OPENING_METHOD',
    department_control:'AUT_AJSI01_DEPARTMENT_CONTROL',
    debit_summary_source:'AUT_AJSI01_DEBIT_SUMMARY_SOURCE',
    credit_summary_source:'AUT_AJSI01_CREDIT_SUMMARY_SOURCE',
    transfer_journal_type:'AUT_AJSI01_TRANSFER_JOURNAL_TYPE',
    transfer_journal_nature:'AUT_AJSI01_TRANSFER_JOURNAL_NATURE'
  };
  async function autoJournalSettingEvent(conn,c,id,kind,before,after,reason,userId) {
    await conn.query(`INSERT INTO accounting_system_parameter_events
      (parameter_id,tenant_id,company_id,source_system,source_database,event_kind,before_json,after_json,reason,user_id)
      VALUES(?,?,?,?,?,?,?,?,?,?)`, [id,...contextParams(c),kind,before ? JSON.stringify(before) : null,after ? JSON.stringify(after) : null,trim(reason)||null,userId||null]);
  }
  async function ensureAutoJournalSettings(conn,c,userId=null) {
    for (const setting of autoJournalSettingDefaults) {
      const [[existing]] = await conn.query(`SELECT id FROM accounting_system_parameters
        WHERE tenant_id=? AND company_id=? AND source_system=? AND source_database=? AND parameter_code=? LIMIT 1`,
        [...contextParams(c),setting.code]);
      if (existing) continue;
      const [result] = await conn.query(`INSERT INTO accounting_system_parameters
        (tenant_id,company_id,source_system,source_database,parameter_code,parameter_name,parameter_value,data_type,
         version_no,status,note,created_by,approved_by,approved_at)
        VALUES(?,?,?,?,?,?,?,?,1,'approved',?,?,?,NOW())`,
        [...contextParams(c),setting.code,setting.name,setting.value,setting.dataType,setting.note,userId||null,userId||null]);
      await autoJournalSettingEvent(conn,c,result.insertId,'seeded',null,{...setting,version_no:1,status:'approved'},'建立 AJSI01 目標端預設參數',userId);
    }
  }
  async function readAutoJournalSettings(conn,c,userId=null) {
    await ensureAutoJournalSettings(conn,c,userId);
    const [rows] = await conn.query(`SELECT * FROM accounting_system_parameters
      WHERE tenant_id=? AND company_id=? AND source_system=? AND source_database=?
        AND parameter_code LIKE 'AUT_AJSI01_%' AND status='approved'
      ORDER BY parameter_code,version_no DESC`, contextParams(c));
    const latest = new Map();
    for (const row of rows) if (!latest.has(row.parameter_code)) latest.set(row.parameter_code,row);
    const get = (code,fallback) => String(latest.get(code)?.parameter_value ?? fallback);
    const classification=get('AUT_AJSI01_ACCOUNTING_CLASSIFICATION','1');
    const postingMethod=get('AUT_AJSI01_POSTING_METHOD','draft_only');
    const draftOpeningMethod=get('AUT_AJSI01_DRAFT_OPENING_METHOD','aggregate');
    if(!['1','2','3','4'].includes(classification))throw badRequest('AJSI01 會計分類方式只能是 1～4');
    if(!['draft_only','draft_and_voucher'].includes(postingMethod))throw badRequest('AJSI01 拋轉方式設定不正確');
    if(!['aggregate','per_document'].includes(draftOpeningMethod))throw badRequest('AJSI01 底稿產生方式只能是彙總或逐張');
    const booleanValue=code=>get(code,'0')==='1';
    return {
      rows:[...latest.values()],
      accounting_classification:classification,
      posting_method:postingMethod,
      aggregate_accounts:booleanValue('AUT_AJSI01_SAME_ACCOUNT_AGGREGATION'),
      draft_opening_method:draftOpeningMethod,
      department_control:booleanValue('AUT_AJSI01_DEPARTMENT_CONTROL'),
      debit_summary_source:get('AUT_AJSI01_DEBIT_SUMMARY_SOURCE','document_no'),
      credit_summary_source:get('AUT_AJSI01_CREDIT_SUMMARY_SOURCE','document_no'),
      transfer_journal_type:get('AUT_AJSI01_TRANSFER_JOURNAL_TYPE','91'),
      transfer_journal_nature:get('AUT_AJSI01_TRANSFER_JOURNAL_NATURE','general')
    };
  }
  function autoJournalSettingInput(body={}) {
    const input=body.settings && typeof body.settings==='object' && !Array.isArray(body.settings) ? body.settings : body;
    const changes=[];
    for(const [rawKey,rawValue] of Object.entries(input||{})){
      const key=autoJournalSettingsByCode.has(rawKey) ? rawKey : autoJournalSettingAliases[String(rawKey).toLowerCase()];
      const definition=key ? autoJournalSettingsByCode.get(key) : null;
      if(!definition)continue;
      let value=String(rawValue ?? '').trim();
      if(key==='AUT_AJSI01_ACCOUNTING_CLASSIFICATION'&&!['1','2','3','4'].includes(value))throw badRequest('會計分類方式只能是 1～4');
      if(key==='AUT_AJSI01_POSTING_METHOD'&&!['draft_only','draft_and_voucher'].includes(value))throw badRequest('拋轉方式只能是底稿或底稿及會計傳票');
      if(key==='AUT_AJSI01_SAME_ACCOUNT_AGGREGATION'||key==='AUT_AJSI01_DEPARTMENT_CONTROL'){
        if(!['0','1','true','false'].includes(value.toLowerCase()))throw badRequest(`${definition.name}只能是 0／1`);
        value=['1','true'].includes(value.toLowerCase())?'1':'0';
      }
      if(key==='AUT_AJSI01_DRAFT_OPENING_METHOD'&&!['aggregate','per_document'].includes(value))throw badRequest('底稿產生方式只能是 aggregate 或 per_document');
      if(key.includes('SUMMARY_SOURCE')&&!['document_no','document_date','memo','department_name'].includes(value))throw badRequest('摘要來源設定不正確');
      if(!value)throw badRequest(`${definition.name}不可空白`);
      changes.push({...definition,value});
    }
    if(!changes.length)throw badRequest('請至少提供一項 AJSI01 設定');
    return changes;
  }
  async function autoDraftLines(conn, context, items, options = {}) {
    const aggregateAccounts = options.aggregateAccounts !== false;
    const groups = [];
    const costGroups = [];
    const addGroup = (accountType, flowKind, flowType, amount, partyCode, source, entryRole = 'main') => {
      ...
      const sourceToken = source && !aggregateAccounts
        ? [source.id || source.source_id || '',source.source_document_id || '',source.source_document_item_id || '',source.document_no || source.source_document_no || ''].join(':')
        : '';
      const key = [entryRole,accountType,flowKind,flowType || '*',sign,partyCode || '',sourceToken].join('|');
  ...
  async function insertDraft(conn, { context, date, sourceKind = 'manual', sourceId = null, sourceDocumentNo = null, accountType = null, partyCode = null, memo = null, lines, sources = [], userId }) {
    ...
    const [header] = await conn.query(`INSERT INTO accounting_drafts
      (tenant_id,company_id,source_system,source_database,draft_no,draft_date,source_kind,source_id,source_document_no,account_type,party_code,
       debit_account_code,debit_account_name,credit_account_code,credit_account_name,amount,status,source_locked,source_locked_at,source_locked_by,memo,created_by)
      VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'draft',?,?,?,?,?)`,
      [context.tenant_id,context.company_id,context.source_system,context.source_database,draftNo,date,sourceKind,sourceId,sourceDocumentNo,accountType,partyCode,
        firstDebit.account_code,firstDebit.account_name,firstCredit.account_code,firstCredit.account_name,debitTotal,sources.length ? 1 : 0,sources.length ? new Date() : null,sources.length ? userId : null,memo || null,userId]);
  ...
  // AUT-G01：AJSI01 參數以既有 G01 版本表保存；新增設定只產生草稿，
  // 必須透過既有 G01 核准 API 才會成為生效版本。
  app.get('/api/accounting/auto-settings', async (req,res,next)=>{try{
    await ensureTargetFinanceWorkflowSchema();
    const db=requestDatabase(req),c=contextFor(db);
    const effective=await tx(async conn=>readAutoJournalSettings(conn,c,req.auth?.id||null));
    const [settings]=await pool.query(`SELECT * FROM accounting_system_parameters
      WHERE tenant_id=? AND company_id=? AND source_system=? AND source_database=?
        AND parameter_code LIKE 'AUT_AJSI01_%' ORDER BY parameter_code,version_no DESC`,contextParams(c));
    res.json({ok:true,data:{source_database:db,settings,effective,nature_catalog:autoJournalNatureCatalog}});
  }catch(e){next(e);}});
  app.post('/api/accounting/auto-settings', async (req,res,next)=>{try{
    await ensureTargetFinanceWorkflowSchema();
    const db=requestDatabase(req),c=contextFor(db),changes=autoJournalSettingInput(req.body||{});
    const effectiveFrom=req.body?.effective_from?validDate(req.body.effective_from):null;
    const effectiveTo=req.body?.effective_to?validDate(req.body.effective_to):null;
    if(effectiveFrom&&effectiveTo&&effectiveFrom>effectiveTo)throw badRequest('AJSI01 生效起日不可晚於迄日');
    const out=await tx(async conn=>{
      const created=[];
      for(const change of changes){
        const [[version]]=await conn.query(`SELECT COALESCE(MAX(version_no),0) version_no FROM accounting_system_parameters
          WHERE tenant_id=? AND company_id=? AND source_system=? AND source_database=? AND parameter_code=?`,[...contextParams(c),change.code]);
        const versionNo=Number(version.version_no||0)+1;
        const [result]=await conn.query(`INSERT INTO accounting_system_parameters
          (tenant_id,company_id,source_system,source_database,parameter_code,parameter_name,parameter_value,data_type,
           effective_from,effective_to,version_no,status,note,created_by)
          VALUES(?,?,?,?,?,?,?,?,?,?,?,'draft',?,?)`,[...contextParams(c),change.code,change.name,change.value,change.dataType,effectiveFrom,effectiveTo,versionNo,trim(req.body?.note)||change.note,req.auth.id]);
        const after={...change,version_no:versionNo,status:'draft',effective_from:effectiveFrom,effective_to:effectiveTo};
        await autoJournalSettingEvent(conn,c,result.insertId,'create',null,after,trim(req.body?.reason)||'建立 AUT-G01 AJSI01 設定草稿',req.auth.id);
        created.push({id:result.insertId,...after});
      }
      return created;
    });
    res.status(201).json({ok:true,data:{source_database:db,status:'draft',settings:out}});
  }catch(e){if(e.code==='ER_DUP_ENTRY')e=badRequest('AJSI01 設定版本已存在，請重新整理後再試');next(e);}});

  // AUT-G02：AJSR01～AJSR03 共用報表 API。報表只讀目標 ERP，
  // 以來源帳款、底稿、底稿來源及正式傳票的既有關聯組成，不新增報表表格。
  app.get('/api/accounting/auto-reports', async (req,res,next)=>{try{
    await ensureTargetFinanceWorkflowSchema();
  ...
  app.post('/api/accounting/drafts/generate', async(req,res,next)=>{try{
    ...
    const out=await tx(async conn=>{
      ...
      for(const item of items)assertChronologicalDate(date,item.document_date,'底稿日期');
      await assertOpenAccountingPeriod(conn,c,date);
      const createOne=async itemSet=>{
      ...
      if(mode==='per_document'){
        // 同一交易內逐張產生；編號規則依序取號，避免並行取號造成相同底稿號碼。
        for(const item of items) drafts.push(await createOne([item]));
      }else drafts.push(await createOne(items));
      const first=drafts[0];
      return {...first,drafts,draft_count:drafts.length,generation_mode:mode,posting_method:settings.posting_method,same_account_aggregation:settings.aggregate_accounts};
    });
    res.status(201).json({ok:true,data:out});
  }catch(e){next(e);}});

  // AUT-G03：清除只允許未核准的底稿，保留底稿／來源／事件資料；已過帳
  // 傳票一律走下方受控沖回，不提供刪除原傳票的捷徑。
  app.post('/api/accounting/drafts/:id/clear', async(req,res,next)=>{try{
    await ensureTargetFinanceWorkflowSchema();
    const db=requestDatabase(req),c=contextFor(db),id=Number(req.params.id),reason=trim(req.body?.reason);
    if(!reason)throw badRequest('自動分錄清除必須輸入原因');
    const out=await tx(async conn=>{
      const draft=await loadDraftHeader(conn,id,c,'draft',true);if(!draft)throw badRequest('只有草稿底稿可以清除，核准後請走還原或沖回');
      await conn.query("UPDATE accounting_drafts SET status='restored',source_locked=0,restore_reason=?,restored_by=?,restored_at=NOW() WHERE id=?",[reason,req.auth.id,id]);
      await conn.query(`INSERT INTO accounting_draft_events(draft_id,event_kind,before_status,after_status,reason,user_id) VALUES(?,'cleared','draft','restored',?,?)`,[id,reason,req.auth.id]);
      return {id,status:'restored',source_locked:false,event_kind:'cleared'};
    });
    res.json({ok:true,data:out});
  }catch(e){next(e);}});
  ...
  // AUT-G03／AJSB22：正式傳票不可直接還原或刪除。建立反向底稿後，
  // 仍須依「維護既有行次 → 核准 → 拋轉」完成沖回；原傳票保持已過帳與可追溯。
  app.post(['/api/accounting/journals/:id/reverse','/api/accounting/journals/:id/restore'], async (req,res,next) => { try {
    await ensureTargetFinanceWorkflowSchema();
    const id=Number(req.params.id),db=requestDatabase(req),c=contextFor(db),reason=trim(req.body?.reason)||'建立會計傳票沖回底稿';
    const out=await tx(async conn=>{
      const [[journal]]=await conn.query(`SELECT * FROM accounting_journals
        WHERE id=? AND tenant_id=? AND company_id=? AND source_system=? AND source_database=? AND status='posted' FOR UPDATE`,
        [id,c.tenant_id,c.company_id,c.source_system,db]);
      if(!journal)throw badRequest('只有目前公司的已過帳傳票可以建立沖回底稿');
      const [[existing]]=await conn.query(`SELECT id,draft_no,status FROM accounting_drafts
        WHERE tenant_id=? AND company_id=? AND source_system=? AND source_database=? AND source_kind='journal_reversal' AND source_id=? AND status IN ('draft','approved','posted') LIMIT 1`,
        [c.tenant_id,c.company_id,c.source_system,db,id]);
      if(existing)throw badRequest(`原傳票已有沖回底稿 ${existing.draft_no}（${existing.status}）`);
      const reversalDate=validDate(req.body?.reversal_date||dateText(journal.journal_date));
      await assertOpenAccountingPeriod(conn,c,reversalDate);
      const [sourceLines]=await conn.query('SELECT * FROM accounting_journal_lines WHERE journal_id=? ORDER BY line_no',[id]);
      if(sourceLines.length<2)throw badRequest('原傳票沒有足夠明細，不能建立沖回底稿');
      const rawLines=sourceLines.map((line,index)=>({
        line_no:index+1,account_code:line.account_code,account_name:line.account_name,
        debit_amount:Number(line.credit_amount||0),credit_amount:Number(line.debit_amount||0),
        party_code:line.party_code,description:`沖回 ${journal.journal_no}／${line.description||''}`.slice(0,500)
      }));
      const checked=await normalizeDraftLines(conn,rawLines,{preserveLineNo:true,context:c});
      const draft=await insertDraft(conn,{context:c,date:reversalDate,sourceKind:'journal_reversal',sourceId:id,sourceDocumentNo:journal.journal_no,accountType:null,partyCode:null,memo:reason,lines:checked.lines,sources:[],userId:req.auth.id});
      return {...draft,original_journal_no:journal.journal_no};
    });
    res.status(201).json({ok:true,data:{...out,original_journal_id:id,original_journal_no:out.original_journal_no||null,action:'reverse'} });
  }catch(e){next(e);}});
...
// G01～G05 是會計總帳管理系統的目標端獨立作業。
// 這些 API 只在目前登入公司對應的 inventory_erp 目標庫執行；
// SH／SC 原始庫不在這裡寫入，也不作為新 ERP 的維護主檔。
function registerAccountingManagementRoutes(app) {
  ...
  const gAccountSeeds = [
    ...
    ['1131','預付貨款','asset','debit'],['1201','商品存貨','asset','debit'],
    ['1501','累計折舊','asset','credit'],['2101','應付帳款','liability','credit'],
    ['2141','應付票據','liability','credit'],['2201','客戶預收款','liability','credit'],
    ['3200','期初餘額調整','equity','credit'],['3201','保留盈餘','equity','credit'],
    ['4101','銷貨收入','revenue','credit'],['5101','銷貨成本','expense','debit'],
    ['7161','兌換利益','revenue','credit'],['7162','兌換損失','expense','debit']
  ];

  const contextOf = req => {
    const key = String(req.erpContext?.source_database || req.query.source_database || req.body?.source_database || 'SH').trim().toUpperCase();
    const source = sourceDatabases[key];
    if (!source) throw badRequest(`不允許的資料來源：${key}`);
    return { tenant_id:source.tenant_id || key, company_id:source.company_id || key,
      source_system:source.source_system || source.adapter_code || 'iSM', source_database:key };
  };
  const json = value => value == null ? null : JSON.stringify(value);
  const optionalText = value => trim(value) || null;
  const dateOrNull = (value, label) => {
    if (value == null || trim(value) === '') return null;
    try { return validDate(value); } catch (_) { throw badRequest(`${label}格式錯誤，必須為 YYYY-MM-DD`); }
  };
  const assertDateRange = (from, to, label='生效期間') => {
    if (from && to && from > to) throw badRequest(`${label}起日不可晚於迄日`);
  };
  const decimal = (value, label, { min = null, max = null } = {}) => {
    const number = Number(value);
    if (!Number.isFinite(number) || (min != null && number < min) || (max != null && number > max)) {
      throw badRequest(`${label}數值不正確`);
    }
    return number;
  };
  const integer = (value, label, { min = null, max = null } = {}) => {
    const number = Number(value);
    if (!Number.isInteger(number) || (min != null && number < min) || (max != null && number > max)) {
      throw badRequest(`${label}必須是有效整數`);
    }
    return number;
  };
  const contextParams = c => [c.tenant_id,c.company_id,c.source_system,c.source_database];
  const stateOf = row => row ? { ...row, id:Number(row.id), version_no:Number(row.version_no || 1) } : null;

  async function ensureGAccounts(conn, c) {
    for (const [code,name,type,normal] of gAccountSeeds) {
  ...
  async function accountMap(conn, c, codes) {
    ...
    return new Map(rows.map(row => [String(row.account_code), row]));
  }
  async function requireAccounts(conn, c, codes, label='科目') {
    const map = await accountMap(conn,c,codes);
    for (const code of [...new Set(codes.map(trim).filter(Boolean))]) {
      const row = map.get(code);
      if (!row || !Number(row.is_active) || String(row.status || 'approved') !== 'approved') throw badRequest(`${label} ${code} 不存在、停用或尚未核准`);
    }
    return map;
  }
  async function parameterEvent(conn, c, id, kind, before, after, reason, userId) {
    await conn.query(`INSERT INTO accounting_system_parameter_events
      (parameter_id,tenant_id,company_id,source_system,source_database,event_kind,before_json,after_json,reason,user_id)
      VALUES(?,?,?,?,?,?,?,?,?,?)`, [id,...contextParams(c),kind,json(before),json(after),optionalText(reason),userId || null]);
  }
  async function accountEvent(conn, c, id, kind, before, after, reason, userId) {
    await conn.query(`INSERT INTO accounting_account_events
      (account_id,tenant_id,company_id,source_system,source_database,event_kind,before_json,after_json,reason,user_id)
      VALUES(?,?,?,?,?,?,?,?,?,?)`, [id,...contextParams(c),kind,json(before),json(after),optionalText(reason),userId || null]);
  }
  async function budgetEvent(conn, c, id, kind, beforeStatus, afterStatus, reason, userId) {
    await conn.query(`INSERT INTO accounting_budget_events
      (budget_id,tenant_id,company_id,source_system,source_database,event_kind,before_status,after_status,reason,user_id)
      VALUES(?,?,?,?,?,?,?,?,?,?)`, [id,...contextParams(c),kind,beforeStatus || null,afterStatus || null,optionalText(reason),userId || null]);
  }
  async function assetEvent(conn, c, id, kind, before, after, reason, userId) {
    await conn.query(`INSERT INTO accounting_fixed_asset_events
      (asset_id,tenant_id,company_id,source_system,source_database,event_kind,before_json,after_json,reason,user_id)
      VALUES(?,?,?,?,?,?,?,?,?,?)`, [id,...contextParams(c),kind,json(before),json(after),optionalText(reason),userId || null]);
  }
  async function profitEvent(conn, c, entityKind, entityId, kind, before, after, reason, userId) {
    await conn.query(`INSERT INTO accounting_profit_center_events
      (entity_kind,entity_id,tenant_id,company_id,source_system,source_database,event_kind,before_json,after_json,reason,user_id)
      VALUES(?,?,?,?,?,?,?,?,?,?,?)`, [entityKind,entityId,...contextParams(c),kind,json(before),json(after),optionalText(reason),userId || null]);
  }
  ...
  function parameterPayload(body) {
    ...
    if (dataType === 'boolean') {
      ...
      if (!['0','1','true','false','yes','no'].includes(normalized)) throw badRequest('布林參數值只能是 0／1 或 true／false');
  ...
  app.get('/api/accounting/g01/parameters', async (req,res,next) => { try {
    await ensureTargetFinanceWorkflowSchema(); const c=contextOf(req);
    const [rows] = await pool.query(`SELECT * FROM accounting_system_parameters
      WHERE tenant_id=? AND company_id=? AND source_system=? AND source_database=?
      ORDER BY parameter_code,version_no DESC`, contextParams(c));
  ...
  app.get('/api/accounting/g01/parameters/:id', async (req,res,next) => { try {
    ...
    if (!row) throw notFound('找不到目前公司／資料來源的會計系統參數');
    const [events] = await pool.query(`SELECT * FROM accounting_system_parameter_events WHERE parameter_id=? ORDER BY id`, [id]);
    res.json({ok:true,data:{...stateOf(row),events}});
  } catch (e) { next(e); } });
  app.post('/api/accounting/g01/parameters', async (req,res,next) => { try {
    ...
    const out=await tx(async conn=>{
      ...
      const versionNo=Number(version.version_no||0)+1;
      const [result] = await conn.query(`INSERT INTO accounting_system_parameters
        (tenant_id,company_id,source_system,source_database,parameter_code,parameter_name,parameter_value,data_type,effective_from,effective_to,version_no,status,note,created_by)
        VALUES(?,?,?,?,?,?,?,?,?,?,?,'draft',?,?)`, [...contextParams(c),payload.code,payload.name,payload.value,payload.dataType,payload.effectiveFrom,payload.effectiveTo,versionNo,payload.note,req.auth.id]);
      await parameterEvent(conn,c,result.insertId,'create',null,{...payload,version_no:versionNo,status:'draft'},'建立 G01 參數',req.auth.id);
      return {id:result.insertId,parameter_code:payload.code,version_no:versionNo,status:'draft'};
    });
    res.status(201).json({ok:true,data:out});
  } catch (e) { if(e.code==='ER_DUP_ENTRY')e=badRequest('同一公司／來源的參數版本已存在'); next(e); } });
  app.put('/api/accounting/g01/parameters/:id', async (req,res,next) => { try {
    await ensureTargetFinanceWorkflowSchema(); const c=contextOf(req), id=Number(req.params.id), payload=parameterPayload(req.body || {});
    const out=await tx(async conn=>{
      ...
      await parameterEvent(conn,c,id,'update',before,after,'修改 G01 參數並建立待核准版本',req.auth.id);
      return stateOf(after);
    });
    res.json({ok:true,data:out});
  } catch (e) { next(e); } });
  app.post('/api/accounting/g01/parameters/:id/approve', async (req,res,next) => { try {
    await ensureTargetFinanceWorkflowSchema(); const c=contextOf(req), id=Number(req.params.id);
    const out=await tx(async conn=>{
      ...
      const [[after]] = await conn.query('SELECT * FROM accounting_system_parameters WHERE id=?',[id]);
      await parameterEvent(conn,c,id,'approve',before,after,'核准 G01 參數',req.auth.id); return stateOf(after);
    });
    res.json({ok:true,data:out});
  } catch (e) { next(e); } });
  app.post('/api/accounting/g01/parameters/:id/void', async (req,res,next) => { try {
    await ensureTargetFinanceWorkflowSchema(); const c=contextOf(req), id=Number(req.params.id), reason=trim(req.body?.reason);
    if(!reason) throw badRequest('停用參數必須輸入原因');
    const out=await tx(async conn=>{
      const [[before]] = await conn.query(`SELECT * FROM accounting_system_parameters WHERE id=? AND tenant_id=? AND company_id=? AND source_system=? AND source_database=? AND status IN ('draft','approved') FOR UPDATE`, [id,...contextParams(c)]);
      if(!before) throw notFound('找不到可停用的 G01 參數');
      await conn.query("UPDATE accounting_system_parameters SET status='voided',note=? WHERE id=?",[reason,id]);
      const [[after]]=await conn.query('SELECT * FROM accounting_system_parameters WHERE id=?',[id]); await parameterEvent(conn,c,id,'void',before,after,reason,req.auth.id); return stateOf(after);
    });
    res.json({ok:true,data:out});
  } catch (e) { next(e); } });

  function accountPayload(body, {create=true}={}) {
    const code=trim(body.account_code), name=trim(body.account_name), type=trim(body.account_type).toLowerCase();
    if(create && !code) throw badRequest('科目代號不可空白'); if(!name) throw badRequest('科目名稱不可空白');
    if(!accountTypes.has(type)) throw badRequest('科目類別不正確');
    const parent=optionalText(body.parent_account_code), level=integer(body.account_level || (parent?2:1),'科目層級',{min:1,max:20});
    const normal=trim(body.normal_balance).toLowerCase() || (['liability','equity','revenue'].includes(type)?'credit':'debit');
    if(!['debit','credit'].includes(normal)) throw badRequest('正常餘額只能是 debit 或 credit');
    const isDetail=Number(body.is_detail ?? 1)?1:0, from=dateOrNull(body.effective_from,'啟用起日'), to=dateOrNull(body.effective_to,'啟用迄日'); assertDateRange(from,to,'科目啟用期間');
    return {code,name,type,parent,level,normal,isDetail,from,to,note:optionalText(body.note)};
  }
  // G02 會計科目設定作業：沿用既有 accounting_accounts，補上階層、期間、狀態與異動歷程。
  app.get('/api/accounting/g02/accounts', async (req,res,next) => { try {
    await ensureTargetFinanceWorkflowSchema(); const c=contextOf(req); await tx(async conn=>ensureGAccounts(conn,c));
  ...
  app.post('/api/accounting/g02/accounts', async(req,res,next)=>{try{
    await ensureTargetFinanceWorkflowSchema();const c=contextOf(req),payload=accountPayload(req.body||{});const out=await tx(async conn=>{await ensureGAccounts(conn,c);if(payload.parent){const parentMap=await accountMap(conn,c,[payload.parent]);if(!parentMap.has(payload.parent))throw badRequest(`上層科目不存在：${payload.parent}`);}const[r]=await conn.query(`INSERT INTO accounting_accounts(tenant_id,company_id,source_system,account_code,account_name,account_type,parent_account_code,account_level,normal_balance,is_detail,effective_from,effective_to,status,is_active,note,updated_by) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,'draft',1,?,?)`,[c.tenant_id,c.company_id,c.source_system,payload.code,payload.name,payload.type,payload.parent,payload.level,payload.normal,payload.isDetail,payload.from,payload.to,payload.note,req.auth.id]);const[[after]]=await conn.query('SELECT * FROM accounting_accounts WHERE id=?',[r.insertId]);await accountEvent(conn,c,r.insertId,'create',null,after,'建立 G02 會計科目草稿',req.auth.id);return after;});res.status(201).json({ok:true,data:out});
  }catch(e){if(e.code==='ER_DUP_ENTRY')e=badRequest('科目代號已存在，請使用修改或建立不同代號');next(e);}});
  app.put('/api/accounting/g02/accounts/:id', async(req,res,next)=>{try{
    await ensureTargetFinanceWorkflowSchema();const c=contextOf(req),id=Number(req.params.id),payload=accountPayload(req.body||{}, {create:false});const out=await tx(async conn=>{const[[before]]=await conn.query(`SELECT * FROM accounting_accounts WHERE id=? AND tenant_id=? AND company_id=? AND source_system=? FOR UPDATE`,[id,c.tenant_id,c.company_id,c.source_system]);if(!before)throw notFound('找不到目前公司的會計科目');if(String(before.account_code)!==payload.code)throw badRequest('會計科目代號不可修改');if(String(before.account_type)!==payload.type){const[[used]]=await conn.query(`SELECT 1 FROM accounting_journal_lines l JOIN accounting_journals j ON j.id=l.journal_id WHERE j.tenant_id=? AND j.company_id=? AND j.source_system=? AND j.source_database=? AND l.account_code=? LIMIT 1`,[...contextParams(c),payload.code]);if(used)throw badRequest('已有傳票使用此科目，類別不可變更');}if(payload.parent){const parents=await accountMap(conn,c,[payload.parent]);if(!parents.has(payload.parent))throw badRequest(`上層科目不存在：${payload.parent}`);}await conn.query(`UPDATE accounting_accounts SET account_name=?,account_type=?,parent_account_code=?,account_level=?,normal_balance=?,is_detail=?,effective_from=?,effective_to=?,version_no=version_no+1,status='draft',approved_by=NULL,approved_at=NULL,is_active=1,note=?,updated_by=? WHERE id=?`,[payload.name,payload.type,payload.parent,payload.level,payload.normal,payload.isDetail,payload.from,payload.to,payload.note,req.auth.id,id]);const[[after]]=await conn.query('SELECT * FROM accounting_accounts WHERE id=?',[id]);await accountEvent(conn,c,id,'update',before,after,'修改 G02 科目並送待核准',req.auth.id);return after;});res.json({ok:true,data:out});
  }catch(e){next(e);}});
  app.post('/api/accounting/g02/accounts/:id/approve',async(req,res,next)=>{try{
    await ensureTargetFinanceWorkflowSchema();const c=contextOf(req),id=Number(req.params.id);const out=await tx(async conn=>{const[[before]]=await conn.query(`SELECT * FROM accounting_accounts WHERE id=? AND tenant_id=? AND company_id=? AND source_system=? AND status='draft' FOR UPDATE`,[id,c.tenant_id,c.company_id,c.source_system]);if(!before)throw badRequest('只有 G02 草稿科目可以核准');if(before.parent_account_code){const parents=await accountMap(conn,c,[before.parent_account_code]);if(!parents.has(String(before.parent_account_code)))throw badRequest(`上層科目不存在：${before.parent_account_code}`);}await conn.query("UPDATE accounting_accounts SET status='approved',is_active=1,approved_by=?,approved_at=NOW() WHERE id=?",[req.auth.id,id]);const[[after]]=await conn.query('SELECT * FROM accounting_accounts WHERE id=?',[id]);await accountEvent(conn,c,id,'approve',before,after,'核准 G02 會計科目',req.auth.id);return after;});res.json({ok:true,data:out});
  }catch(e){next(e);}});
  app.post('/api/accounting/g02/accounts/:id/disable',async(req,res,next)=>{try{
    await ensureTargetFinanceWorkflowSchema();const c=contextOf(req),id=Number(req.params.id),reason=trim(req.body?.reason);if(!reason)throw badRequest('停用科目必須輸入原因');const out=await tx(async conn=>{const[[before]]=await conn.query(`SELECT * FROM accounting_accounts WHERE id=? AND tenant_id=? AND company_id=? AND source_system=? FOR UPDATE`,[id,c.tenant_id,c.company_id,c.source_system]);if(!before)throw notFound('找不到目前公司的會計科目');await conn.query("UPDATE accounting_accounts SET is_active=0,status='disabled',note=?,updated_by=? WHERE id=?",[reason,req.auth.id,id]);const[[after]]=await conn.query('SELECT * FROM accounting_accounts WHERE id=?',[id]);await accountEvent(conn,c,id,'disable',before,after,reason,req.auth.id);return after;});res.json({ok:true,data:out});
  }catch(e){next(e);}});

  function budgetLines(body, periodCount) {
    let raw=body.lines;
    if(!Array.isArray(raw) && trim(body.lines_json)){try{raw=JSON.parse(body.lines_json);}catch(_){throw badRequest('預算明細 JSON 格式錯誤');}}
    if(!Array.isArray(raw)||!raw.length)throw badRequest('預算至少需要一筆科目／部門明細');
    if(raw.length>1000)throw badRequest('單一預算明細不可超過 1000 筆');
    const seen=new Set();return raw.map((line,index)=>{const lineNo=integer(line.line_no??index+1,'預算行次',{min:1});if(seen.has(lineNo))throw badRequest(`預算行次重複：${lineNo}`);seen.add(lineNo);const periodNo=integer(line.period_no??0,'預算期間',{min:0,max:periodCount});const amount=decimal(line.budget_amount??line.amount??0,'預算金額',{min:0});const accountCode=trim(line.account_code);if(!accountCode)throw badRequest(`第 ${index+1} 筆預算缺少科目`);return{line_no:lineNo,account_code:accountCode,account_name:trim(line.account_name)||null,department_code:optionalText(line.department_code),period_no:periodNo,budget_amount:amount,note:optionalText(line.note)};});
  }
  async function insertBudgetLines(conn,id,lines){for(const line of lines)await conn.query(`INSERT INTO accounting_budget_lines(budget_id,line_no,account_code,account_name,department_code,period_no,budget_amount,note) VALUES(?,?,?,?,?,?,?,?)`,[id,line.line_no,line.account_code,line.account_name,line.department_code,line.period_no,line.budget_amount,line.note]);}
  async function budgetDetail(conn,c,id){const[[header]]=await conn.query(`SELECT * FROM accounting_budgets WHERE id=? AND tenant_id=? AND company_id=? AND source_system=? AND source_database=?`,[id,...contextParams(c)]);if(!header)throw notFound('找不到目前公司／資料來源的預算');const[lines]=await conn.query('SELECT * FROM accounting_budget_lines WHERE budget_id=? ORDER BY line_no',[id]);return{...header,lines};}
  const G03_EPS=0.000001;
  function g03DateRange(year,fromValue,toValue){
    const from=fromValue?validDate(fromValue):`${year}-01-01`,to=toValue?validDate(toValue):`${year}-12-31`;
    if(from>to)throw badRequest('預算報表日期起日不可晚於迄日');
    if(from.slice(0,4)!==year||to.slice(0,4)!==year)throw badRequest('預算報表日期區間不可跨會計年度');
    return{from,to};
  }
  function g03Time(value){const time=value?new Date(value).getTime():0;return Number.isFinite(time)?time:0;}
  function g03LatestBudgetIds(rows){
    const latest=new Map();
    for(const row of rows){
      const key=`${row.budget_code}|${row.currency_code||''}`;const current=latest.get(key);
      if(!current||g03Time(row.approved_at)>g03Time(current.approved_at)||
        (g03Time(row.approved_at)===g03Time(current.approved_at)&&g03Time(row.updated_at)>g03Time(current.updated_at))||
        (g03Time(row.approved_at)===g03Time(current.approved_at)&&g03Time(row.updated_at)===g03Time(current.updated_at)&&Number(row.budget_id)>Number(current.budget_id))) latest.set(key,row);
    }
    return new Set([...latest.values()].map(row=>Number(row.budget_id)));
  }
  function g03EffectiveRows(rows){
    const groups=new Map();
    for(const row of rows){const key=`${row.budget_id}|${row.account_code}|${row.department_code||''}`;const list=groups.get(key)||[];list.push(row);groups.set(key,list);}
    const chosen=new Set();
    for(const list of groups.values()){
      const hasMonthly=list.some(row=>Number(row.period_no)>0);
      for(const row of list)if(!hasMonthly||Number(row.period_no)>0)chosen.add(Number(row.line_id));
    }
    return rows.filter(row=>chosen.has(Number(row.line_id)));
  }
  function g03Sum(rows,field){return rows.reduce((sum,row)=>sum+Number(row[field]||0),0);}
  function g03Group(rows,keyOf,make){
    const map=new Map();
    for(const row of rows){const key=keyOf(row),current=map.get(key)||make(row);current.budget_amount+=Number(row.budget_amount||0);current.actual_amount+=Number(row.actual_amount||0);map.set(key,current);}
    return[...map.values()].map(row=>({...row,variance_amount:row.actual_amount-row.budget_amount,available_amount:row.budget_amount-row.actual_amount}));
  }
  async function g03BuildReportData(conn,c,options={}){
    const year=trim(options.fiscalYear);if(!/^\d{4}$/.test(year))throw badRequest('預算報表必須指定四碼會計年度');
    const dates=g03DateRange(year,options.dateFrom,options.dateTo),budgetWhere=['b.tenant_id=?','b.company_id=?','b.source_system=?','b.source_database=?','b.fiscal_year=?',"b.status='approved'"];const budgetParams=[...contextParams(c),year];
    const budgetCode=trim(options.budgetCode),versionName=trim(options.versionName),departmentCode=trim(options.departmentCode),accountCode=trim(options.accountCode),currencyCode=trim(options.currencyCode);
    if(budgetCode){budgetWhere.push('b.budget_code=?');budgetParams.push(budgetCode);}if(versionName){budgetWhere.push('b.version_name=?');budgetParams.push(versionName);}if(departmentCode){budgetWhere.push("COALESCE(l.department_code,'')=?");budgetParams.push(departmentCode==='******'?'':departmentCode);}if(accountCode){budgetWhere.push('l.account_code=?');budgetParams.push(accountCode);}if(currencyCode){budgetWhere.push('b.currency_code=?');budgetParams.push(currencyCode);}
    if(options.periodNo!==undefined&&options.periodNo!==null&&trim(options.periodNo)!==''){const period=integer(options.periodNo,'預算期間',{min:0,max:13});budgetWhere.push('l.period_no=?');budgetParams.push(period);}
    const[rawBudgetRows]=await conn.query(`SELECT b.id budget_id,b.budget_code,b.budget_name,b.fiscal_year,b.version_name,b.period_count,b.currency_code,b.status,b.total_amount,b.approved_at,b.updated_at,
        l.id line_id,l.line_no,l.account_code,l.account_name,l.department_code,l.period_no,l.budget_amount,
        COALESCE(a.account_type,'unknown') account_type,a.normal_balance,a.is_detail,a.is_active account_active,COALESCE(a.status,'missing') account_status
    ...
    const latestIds=g03LatestBudgetIds(rawBudgetRows),selectedIds=options.latestOnly&&!versionName?latestIds:new Set(rawBudgetRows.map(row=>Number(row.budget_id))),selectedBudgetRows=rawBudgetRows.filter(row=>selectedIds.has(Number(row.budget_id)));
    const actualWhere=['j.tenant_id=?','j.company_id=?','j.source_system=?','j.source_database=?',"j.status='posted'",'j.journal_date BETWEEN ? AND ?'];const actualParams=[...contextParams(c),dates.from,dates.to];if(accountCode){actualWhere.push('l.account_code=?');actualParams.push(accountCode);}if(departmentCode){actualWhere.push("COALESCE(l.department_code,'')=?");actualParams.push(departmentCode==='******'?'':departmentCode);}
    const[actualRaw]=await conn.query(`SELECT l.account_code,l.account_name,COALESCE(l.department_code,'') department_code,COALESCE(l.profit_center_code,'') profit_center_code,MONTH(j.journal_date) period_no,
        COALESCE(a.account_type,'unknown') account_type,a.normal_balance,COUNT(DISTINCT j.id) journal_count,
      ...
      WHERE ${actualWhere.join(' AND ')} GROUP BY l.account_code,l.account_name,COALESCE(l.department_code,''),COALESCE(l.profit_center_code,''),MONTH(j.journal_date),COALESCE(a.account_type,'unknown'),a.normal_balance`,actualParams);
    const actualRows=actualRaw.map(row=>({...row,department_code:row.department_code||null,profit_center_code:row.profit_center_code||null,period_no:Number(row.period_no||0),journal_count:Number(row.journal_count||0),actual_amount:Number(row.actual_amount||0)}));
    const actualFor=(row,periodOverride=null)=>{const account=String(row.account_code),department=String(row.department_code||''),period=periodOverride===null?Number(row.period_no||0):Number(periodOverride);return g03Sum(actualRows.filter(item=>String(item.account_code)===account&&(!department||String(item.department_code||'')===department)&&(!period||Number(item.period_no)===period)),'actual_amount');};
    const rows=selectedBudgetRows.map(row=>{const budget=Number(row.budget_amount||0),actual=actualFor(row);return{...row,budget_amount:budget,actual_amount:actual,variance_amount:actual-budget,available_amount:budget-actual,achievement_rate:budget?actual/budget:null,period_label:Number(row.period_no||0)?`第${row.period_no}期`:'年度',budget_control_applicable:String(row.account_type)==='expense',control_status:String(row.account_type)==='expense'&&actual-budget>G03_EPS?'over_budget':'within_budget',source_table:'accounting_budgets／accounting_budget_lines',source_database:c.source_database};});
    const effectiveRows=g03EffectiveRows(rows),scopeAccounts=new Set(rows.map(row=>String(row.account_code))),scopeDepartments=new Map();for(const row of rows){const key=String(row.account_code);const set=scopeDepartments.get(key)||{all:false,values:new Set()};if(!row.department_code)set.all=true;else set.values.add(String(row.department_code));scopeDepartments.set(key,set);}const actualScope=actualRows.filter(row=>scopeAccounts.has(String(row.account_code))&&(!scopeDepartments.get(String(row.account_code))||scopeDepartments.get(String(row.account_code)).all||scopeDepartments.get(String(row.account_code)).values.has(String(row.department_code||''))));
    const periodCount=Math.max(12,...selectedBudgetRows.map(row=>Number(row.period_count||12))),periodSummaries=[];for(let period=1;period<=periodCount;period++){const budget=g03Sum(effectiveRows.filter(row=>Number(row.period_no)===period),'budget_amount'),actual=g03Sum(actualScope.filter(row=>Number(row.period_no)===period),'actual_amount');periodSummaries.push({period_no:period,period_label:`第${period}期`,budget_amount:budget,actual_amount:actual,variance_amount:actual-budget,available_amount:budget-actual});}const annualBudget=g03Sum(effectiveRows.filter(row=>Number(row.period_no)===0),'budget_amount')||g03Sum(effectiveRows.filter(row=>Number(row.period_no)>0),'budget_amount'),annualActual=g03Sum(actualScope,'actual_amount');periodSummaries.unshift({period_no:0,period_label:'年度',budget_amount:annualBudget,actual_amount:annualActual,variance_amount:annualActual-annualBudget,available_amount:annualBudget-annualActual});
    const versionMap=new Map();for(const row of rawBudgetRows){if(!versionMap.has(Number(row.budget_id)))versionMap.set(Number(row.budget_id),{id:Number(row.budget_id),budget_code:row.budget_code,budget_name:row.budget_name,fiscal_year:row.fiscal_year,version_name:row.version_name,period_count:Number(row.period_count||12),currency_code:row.currency_code,status:row.status,total_amount:Number(row.total_amount||0),approved_at:row.approved_at,updated_at:row.updated_at,line_count:0,selected:selectedIds.has(Number(row.budget_id)),is_latest:latestIds.has(Number(row.budget_id))});versionMap.get(Number(row.budget_id)).line_count++;}
    const versionGroups=new Map();for(const row of rawBudgetRows){const key=[row.budget_code,row.currency_code||'',row.account_code,row.department_code||'',Number(row.period_no||0)].join('|');const list=versionGroups.get(key)||[];list.push(row);versionGroups.set(key,list);}const versionDifferences=[];for(const list of versionGroups.values()){const latest=list.find(row=>latestIds.has(Number(row.budget_id)));if(!latest)continue;for(const row of list){if(Number(row.budget_id)===Number(latest.budget_id))continue;versionDifferences.push({budget_code:latest.budget_code,currency_code:latest.currency_code,account_code:latest.account_code,account_name:latest.account_name,department_code:latest.department_code,period_no:Number(latest.period_no||0),latest_version:latest.version_name,comparison_version:row.version_name,latest_amount:Number(latest.budget_amount||0),comparison_amount:Number(row.budget_amount||0),difference_amount:Number(latest.budget_amount||0)-Number(row.budget_amount||0)});}}
    const dimensionSummary=g03Group(effectiveRows,row=>`${row.account_code}|${row.department_code||''}`,row=>({account_code:row.account_code,account_name:row.account_name,department_code:row.department_code||null,account_type:row.account_type,budget_amount:0,actual_amount:0}));
    const actualDimensionSummary=g03Group(actualScope,row=>`${row.account_code}|${row.department_code||''}|${row.profit_center_code||''}`,row=>({account_code:row.account_code,account_name:row.account_name,department_code:row.department_code||null,profit_center_code:row.profit_center_code||null,account_type:row.account_type,budget_amount:0,actual_amount:0}));
    const warnings=[];if(!rows.length)warnings.push({code:'NO_APPROVED_BUDGET',severity:'warning',message:'目前篩選條件沒有已核准預算，不能視為已完成預算控制'});if(rows.some(row=>!row.account_active||row.account_status!=='approved'))warnings.push({code:'ACCOUNT_NOT_READY',severity:'error',message:'預算明細含不存在、停用或尚未核准的會計科目'});if(rows.length)warnings.push({code:'MAIN_LEDGER_SCOPE_UNREPRESENTED',severity:'warning',message:'既有會計科目結構沒有文件所稱主要帳別／科目部門設限欄位；目前僅檢核科目存在、明細、核准與啟用，不猜測限制'});
    const totalRows=effectiveRows,overBudget=totalRows.filter(row=>row.control_status==='over_budget');return{source_database:c.source_database,company_id:c.company_id,fiscal_year:year,date_from:dates.from,date_to:dates.to,filters:{budget_code:budgetCode||null,version_name:versionName||null,department_code:departmentCode||null,account_code:accountCode||null,currency_code:currencyCode||null,period_no:options.periodNo===undefined||options.periodNo===null||trim(options.periodNo)===''?null:Number(options.periodNo)},version_selection:options.latestOnly&&!versionName?'latest_approved_per_budget_code':'all_matching_approved_versions',period_count:periodCount,budgets:[...versionMap.values()],rows,actual_rows:actualRows,period_summaries:periodSummaries,dimension_summary:dimensionSummary,actual_dimension_summary:actualDimensionSummary,version_differences:versionDifferences,warnings,summary:{budget_total:g03Sum(totalRows,'budget_amount'),actual_total:g03Sum(totalRows,'actual_amount'),variance_total:g03Sum(totalRows,'variance_amount'),available_total:g03Sum(totalRows,'available_amount'),over_budget_count:overBudget.length,over_budget_amount:overBudget.reduce((sum,row)=>sum+Math.max(-Number(row.available_amount||0),0),0),healthy:overBudget.length===0&&warnings.every(row=>row.severity!=='error')},effective_row_count:totalRows.length};
  }
  // G03 預算管理：預算名稱、科目／部門預算、複製版本、月份／年度比較與控制檢核。
  app.get('/api/accounting/g03/budgets',async(req,res,next)=>{try{await ensureTargetFinanceWorkflowSchema();const c=contextOf(req),year=trim(req.query.fiscal_year);const where=['tenant_id=?','company_id=?','source_system=?','source_database=?'],params=[...contextParams(c)];if(year){if(!/^\d{4}$/.test(year))throw badRequest('會計年度必須是四碼');where.push('fiscal_year=?');params.push(year);}const[rows]=await pool.query(`SELECT * FROM accounting_budgets WHERE ${where.join(' AND ')} ORDER BY fiscal_year DESC,budget_code,version_name`,params);res.json({ok:true,data:rows,source_database:c.source_database});}catch(e){next(e);}});
  app.get('/api/accounting/g03/budgets/:id',async(req,res,next)=>{try{await ensureTargetFinanceWorkflowSchema();const c=contextOf(req);res.json({ok:true,data:await budgetDetail(pool,c,Number(req.params.id))});}catch(e){next(e);}});
  app.post('/api/accounting/g03/budgets',async(req,res,next)=>{try{await ensureTargetFinanceWorkflowSchema();const c=contextOf(req),b=req.body||{},code=trim(b.budget_code),name=trim(b.budget_name),year=trim(b.fiscal_year),version=trim(b.version_name)||'BASE',periodCount=integer(b.period_count??12,'會計期間數',{min:12,max:13});if(!code||!name||!/^\d{4}$/.test(year))throw badRequest('預算代碼、名稱與四碼會計年度不可空白');const lines=budgetLines(b,periodCount);const out=await tx(async conn=>{await ensureGAccounts(conn,c);const accounts=await requireAccounts(conn,c,lines.map(x=>x.account_code),'預算科目');for(const line of lines){if(!line.account_name)line.account_name=accounts.get(line.account_code).account_name;}const total=lines.reduce((s,x)=>s+x.budget_amount,0);const[r]=await conn.query(`INSERT INTO accounting_budgets(tenant_id,company_id,source_system,source_database,budget_code,budget_name,fiscal_year,version_name,period_count,currency_code,status,total_amount,note,created_by) VALUES(?,?,?,?,?,?,?,?,?,?, 'draft',?,?,?)`,[...contextParams(c),code,name,year,version,periodCount,trim(b.currency_code)||'TWD',total,optionalText(b.note),req.auth.id]);await insertBudgetLines(conn,r.insertId,lines);await budgetEvent(conn,c,r.insertId,'create',null,'draft','建立 G03 預算草稿',req.auth.id);return await budgetDetail(conn,c,r.insertId);});res.status(201).json({ok:true,data:out});}catch(e){if(e.code==='ER_DUP_ENTRY')e=badRequest('同一公司／來源的預算代碼與版本已存在');next(e);}});
  app.put('/api/accounting/g03/budgets/:id',async(req,res,next)=>{try{await ensureTargetFinanceWorkflowSchema();const c=contextOf(req),id=Number(req.params.id),b=req.body||{},periodCount=integer(b.period_count??12,'會計期間數',{min:12,max:13}),lines=budgetLines(b,periodCount);const out=await tx(async conn=>{const[[before]]=await conn.query(`SELECT * FROM accounting_budgets WHERE id=? AND tenant_id=? AND company_id=? AND source_system=? AND source_database=? AND status='draft' FOR UPDATE`,[id,...contextParams(c)]);if(!before)throw badRequest('只有 G03 草稿預算可以修改');const accounts=await requireAccounts(conn,c,lines.map(x=>x.account_code),'預算科目');for(const line of lines)if(!line.account_name)line.account_name=accounts.get(line.account_code).account_name;const total=lines.reduce((s,x)=>s+x.budget_amount,0);await conn.query(`UPDATE accounting_budgets SET budget_name=?,fiscal_year=?,version_name=?,period_count=?,currency_code=?,total_amount=?,note=? WHERE id=?`,[trim(b.budget_name)||before.budget_name,trim(b.fiscal_year)||before.fiscal_year,trim(b.version_name)||before.version_name,periodCount,trim(b.currency_code)||before.currency_code,total,optionalText(b.note),id]);await conn.query('DELETE FROM accounting_budget_lines WHERE budget_id=?',[id]);await insertBudgetLines(conn,id,lines);await budgetEvent(conn,c,id,'update',before.status,'draft','修改 G03 預算明細',req.auth.id);return await budgetDetail(conn,c,id);});res.json({ok:true,data:out});}catch(e){next(e);}});
  app.post('/api/accounting/g03/budgets/:id/approve',async(req,res,next)=>{try{await ensureTargetFinanceWorkflowSchema();const c=contextOf(req),id=Number(req.params.id);const out=await tx(async conn=>{const[[before]]=await conn.query(`SELECT * FROM accounting_budgets WHERE id=? AND tenant_id=? AND company_id=? AND source_system=? AND source_database=? AND status='draft' FOR UPDATE`,[id,...contextParams(c)]);if(!before)throw badRequest('只有 G03 草稿預算可以核准');const[lines]=await conn.query('SELECT * FROM accounting_budget_lines WHERE budget_id=?',[id]);if(!lines.length)throw badRequest('預算沒有明細不可核准');await requireAccounts(conn,c,lines.map(x=>x.account_code),'預算科目');await conn.query("UPDATE accounting_budgets SET status='approved',approved_by=?,approved_at=NOW() WHERE id=?",[req.auth.id,id]);await budgetEvent(conn,c,id,'approve',before.status,'approved','核准 G03 預算',req.auth.id);return await budgetDetail(conn,c,id);});res.json({ok:true,data:out});}catch(e){next(e);}});
  ...
  app.get('/api/accounting/g03/report',async(req,res,next)=>{try{await ensureTargetFinanceWorkflowSchema();const c=contextOf(req),data=await g03BuildReportData(pool,c,{fiscalYear:req.query.fiscal_year,dateFrom:req.query.date_from||req.query.from_date,dateTo:req.query.date_to||req.query.to_date,budgetCode:req.query.budget_code,versionName:req.query.version_name,departmentCode:req.query.department_code,accountCode:req.query.account_code,currencyCode:req.query.currency_code,periodNo:req.query.period_no,latestOnly:false});res.json({ok:true,data});}catch(e){next(e);}});
  // G03-R1：依已核准版本檢核目前／提案金額。此端點只讀，不會偷偷修改來源或預算。
  app.get('/api/accounting/g03/control',async(req,res,next)=>{try{await ensureTargetFinanceWorkflowSchema();const c=contextOf(req),data=await g03BuildReportData(pool,c,{fiscalYear:req.query.fiscal_year,dateFrom:req.query.date_from||req.query.from_date,dateTo:req.query.date_to||req.query.to_date,budgetCode:req.query.budget_code,versionName:req.query.version_name,departmentCode:req.query.department_code,accountCode:req.query.account_code,currencyCode:req.query.currency_code,periodNo:req.query.period_no,latestOnly:true}),selectedIds=new Set(data.rows.map(row=>Number(row.budget_id)));res.json({ok:true,data:{...data,budgets:data.budgets.filter(row=>selectedIds.has(Number(row.id))),control_mode:'latest_approved_per_budget_code',interception:'read_only_check'}});}catch(e){next(e);}});
  app.post('/api/accounting/g03/control/check',async(req,res,next)=>{try{await ensureTargetFinanceWorkflowSchema();const b=req.body||{},c=contextOf(req),year=trim(b.fiscal_year);if(!/^\d{4}$/.test(year))throw badRequest('超預算檢核必須指定四碼會計年度');const budgetCode=trim(b.budget_code);if(!budgetCode)throw badRequest('超預算檢核必須指定預算代碼');const raw=Array.isArray(b.lines)?b.lines:[];if(!raw.length)throw badRequest('超預算檢核至少需要一筆提案明細');if(raw.length>1000)throw badRequest('單次超預算檢核不可超過 1000 筆');const data=await g03BuildReportData(pool,c,{fiscalYear:year,dateFrom:b.date_from,dateTo:b.date_to,budgetCode,versionName:b.version_name,departmentCode:b.department_code,accountCode:b.account_code,currencyCode:b.currency_code,latestOnly:true});const periodCount=data.period_count,issues=[],checks=raw.map((line,index)=>{const account=trim(line.account_code);if(!account)throw badRequest(`第 ${index+1} 筆提案缺少科目`);const period=integer(line.period_no??1,'提案期間',{min:0,max:periodCount}),amount=decimal(line.amount??line.budget_amount,'提案金額',{min:0});const department=trim(line.department_code);let candidates=data.rows.filter(row=>String(row.account_code)===account&&(!department? !row.department_code:String(row.department_code||'')===department));const exact=candidates.filter(row=>Number(row.period_no)===period);if(period>0&&exact.length)candidates=exact;else if(period>0){const annual=candidates.filter(row=>Number(row.period_no)===0);if(annual.length)candidates=annual;}else candidates=exact;const hasBudget=candidates.length>0,budget=candidates.reduce((sum,row)=>sum+Number(row.budget_amount||0),0),actual=candidates.reduce((sum,row)=>sum+Number(row.actual_amount||0),0),after=budget-actual-amount,accountType=candidates[0]?.account_type||'unknown',overBudget=hasBudget&&String(accountType)==='expense'&&after< -G03_EPS;let controlStatus='within_budget';if(!hasBudget){controlStatus='no_budget';issues.push({line_no:index+1,code:'NO_BUDGET',message:`科目 ${account}／期間 ${period} 沒有可用的已核准預算`});}else if(overBudget){controlStatus='over_budget';issues.push({line_no:index+1,code:'OVER_BUDGET',message:`科目 ${account}／期間 ${period} 提案後超出預算 ${Math.abs(after).toFixed(6)}`});}return{line_no:index+1,account_code:account,department_code:department||null,period_no:period,amount,budget_amount:budget,actual_amount:actual,available_before:budget-actual,available_after:after,account_type:accountType,control_status:controlStatus,budget_found:hasBudget};});const blocked=issues.length>0;res.json({ok:true,data:{...data,budget_code:budgetCode,proposal:checks,interception:'read_only_check',allowed:!blocked,blocked,requires_approval:blocked,summary:{...data.summary,proposal_total:checks.reduce((sum,row)=>sum+row.amount,0),proposal_over_budget_count:checks.filter(row=>row.control_status==='over_budget').length,proposal_missing_budget_count:checks.filter(row=>row.control_status==='no_budget').length}}});}catch(e){next(e);}});
  ...
  function assetPayload(body) {
    ...
    const accountCode=trim(body.account_code)||'1501', accountName=trim(body.account_name)||'累計折舊';
    const expenseCode=trim(body.depreciation_expense_account_code)||'5101', expenseName=trim(body.depreciation_expense_account_name)||'銷貨成本';
    const original=decimal(body.original_cost,'資產原始成本',{min:0.000001}), residual=decimal(body.residual_value??0,'資產殘值',{min:0});
    if(residual>original)throw badRequest('資產殘值不可大於原始成本');
    const life=integer(body.useful_life_months,'耐用年限（月）',{min:1,max:1200});
    const inService=validDate(body.in_service_date), acquisition=dateOrNull(body.acquisition_date,'取得日期');
    if(acquisition&&acquisition>inService)throw badRequest('取得日期不可晚於啟用日期');
  ...
  app.post('/api/accounting/g04/assets',async(req,res,next)=>{try{await ensureTargetFinanceWorkflowSchema();const c=contextOf(req),payload=assetPayload(req.body||{});const out=await tx(async conn=>{await ensureGAccounts(conn,c);await requireAccounts(conn,c,[payload.accountCode,payload.expenseCode],'固定資產科目');const[r]=await conn.query(`INSERT INTO accounting_fixed_assets(tenant_id,company_id,source_system,source_database,asset_no,asset_name,category_code,account_code,account_name,depreciation_expense_account_code,depreciation_expense_account_name,department_code,location,supplier_code,acquisition_date,in_service_date,original_cost,residual_value,useful_life_months,depreciation_method,currency_code,accumulated_depreciation,net_book_value,status,note,created_by) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,0,?,'draft',?,?)`,[...contextParams(c),payload.assetNo,payload.assetName,payload.categoryCode,payload.accountCode,payload.accountName,payload.expenseCode,payload.expenseName,payload.departmentCode,payload.location,payload.supplierCode,payload.acquisition,payload.inService,payload.original,payload.residual,payload.life,payload.method,payload.currency,payload.original,payload.note,req.auth.id]);const[[after]]=await conn.query('SELECT * FROM accounting_fixed_assets WHERE id=?',[r.insertId]);await assetEvent(conn,c,r.insertId,'create',null,after,'建立 G04 固定資產草稿',req.auth.id);return after;});res.status(201).json({ok:true,data:out});}catch(e){if(e.code==='ER_DUP_ENTRY')e=badRequest('固定資產代號已存在');next(e);}});
  app.put('/api/accounting/g04/assets/:id',async(req,res,next)=>{try{await ensureTargetFinanceWorkflowSchema();const c=contextOf(req),id=Number(req.params.id),payload=assetPayload(req.body||{});const out=await tx(async conn=>{const[[before]]=await conn.query(`SELECT * FROM accounting_fixed_assets WHERE id=? AND tenant_id=? AND company_id=? AND source_system=? AND source_database=? AND status='draft' FOR UPDATE`,[id,...contextParams(c)]);if(!before)throw badRequest('只有 G04 草稿固定資產可以修改');await ensureGAccounts(conn,c);await requireAccounts(conn,c,[payload.accountCode,payload.expenseCode],'固定資產科目');await conn.query(`UPDATE accounting_fixed_assets SET asset_name=?,category_code=?,account_code=?,account_name=?,depreciation_expense_account_code=?,depreciation_expense_account_name=?,department_code=?,location=?,supplier_code=?,acquisition_date=?,in_service_date=?,original_cost=?,residual_value=?,useful_life_months=?,depreciation_method=?,currency_code=?,net_book_value=?,note=?,created_by=? WHERE id=?`,[payload.assetName,payload.categoryCode,payload.accountCode,payload.accountName,payload.expenseCode,payload.expenseName,payload.departmentCode,payload.location,payload.supplierCode,payload.acquisition,payload.inService,payload.original,payload.residual,payload.life,payload.method,payload.currency,Math.max(payload.original-Number(before.accumulated_depreciation||0),0),payload.note,req.auth.id,id]);const[[after]]=await conn.query('SELECT * FROM accounting_fixed_assets WHERE id=?',[id]);await assetEvent(conn,c,id,'update',before,after,'修改 G04 固定資產草稿',req.auth.id);return after;});res.json({ok:true,data:out});}catch(e){next(e);}});
  app.post('/api/accounting/g04/assets/:id/approve',async(req,res,next)=>{try{await ensureTargetFinanceWorkflowSchema();const c=contextOf(req),id=Number(req.params.id);const out=await tx(async conn=>{const[[before]]=await conn.query(`SELECT * FROM accounting_fixed_assets WHERE id=? AND tenant_id=? AND company_id=? AND source_system=? AND source_database=? AND status='draft' FOR UPDATE`,[id,...contextParams(c)]);if(!before)throw badRequest('只有 G04 草稿固定資產可以核准');await requireAccounts(conn,c,[before.account_code,before.depreciation_expense_account_code],'固定資產科目');await conn.query("UPDATE accounting_fixed_assets SET status='approved',approved_by=?,approved_at=NOW() WHERE id=?",[req.auth.id,id]);const[[after]]=await conn.query('SELECT * FROM accounting_fixed_assets WHERE id=?',[id]);await assetEvent(conn,c,id,'approve',before,after,'核准 G04 固定資產',req.auth.id);return after;});res.json({ok:true,data:out});}catch(e){next(e);}});
  app.get('/api/accounting/g04/depreciation',async(req,res,next)=>{try{await ensureTargetFinanceWorkflowSchema();const c=contextOf(req),[rows]=await pool.query(`SELECT d.*,a.asset_no,a.asset_name,x.draft_no,x.status draft_status,j.journal_no FROM accounting_fixed_asset_depreciations d JOIN accounting_fixed_assets a ON a.id=d.asset_id LEFT JOIN accounting_drafts x ON x.id=d.journal_draft_id LEFT JOIN accounting_journals j ON j.source_kind='fixed_asset_depreciation' AND j.source_id=d.id AND j.status<>'voided' WHERE d.tenant_id=? AND d.company_id=? AND d.source_system=? AND d.source_database=? ORDER BY d.depreciation_period DESC,d.id DESC`,contextParams(c));res.json({ok:true,data:rows,source_database:c.source_database});}catch(e){next(e);}});
  app.post('/api/accounting/g04/depreciation',async(req,res,next)=>{try{await ensureTargetFinanceWorkflowSchema();const c=contextOf(req),assetId=integer(req.body?.asset_id,'固定資產',{min:1}),period=trim(req.body?.depreciation_period);if(!/^\d{4}-(0[1-9]|1[0-2])$/.test(period))throw badRequest('折舊期間必須是 YYYY-MM');const [year,month]=period.split('-').map(Number),lastDate=new Date(Date.UTC(year,month,0)).toISOString().slice(0,10);const out=await tx(async conn=>{const[[asset]]=await conn.query(`SELECT * FROM accounting_fixed_assets WHERE id=? AND tenant_id=? AND company_id=? AND source_system=? AND source_database=? AND status='approved' FOR UPDATE`,[assetId,...contextParams(c)]);if(!asset)throw badRequest('只有目前公司已核准的固定資產可以計提折舊');await assertOpenAccountingPeriod(conn,c,lastDate);const[[existing]]=await conn.query(`SELECT id FROM accounting_fixed_asset_depreciations WHERE asset_id=? AND tenant_id=? AND company_id=? AND source_system=? AND source_database=? AND depreciation_period=? AND status<>'voided'`,[assetId,...contextParams(c),period]);if(existing)throw badRequest('此固定資產與折舊期間已產生折舊資料');const[[pending]]=await conn.query(`SELECT id,depreciation_period FROM accounting_fixed_asset_depreciations WHERE asset_id=? AND tenant_id=? AND company_id=? AND source_system=? AND source_database=? AND status IN ('draft','approved') AND depreciation_period<? LIMIT 1`,[assetId,...contextParams(c),period]);if(pending)throw badRequest(`前期折舊 ${pending.depreciation_period} 尚未過帳`);const original=Number(asset.original_cost),residual=Number(asset.residual_value||0),accumulated=Number(asset.accumulated_depreciation||0),remaining=Math.max(original-residual-accumulated,0),monthly=(original-residual)/Number(asset.useful_life_months);if(remaining<=0.000001)throw badRequest('固定資產已提足折舊');const amount=Math.min(monthly,remaining),newAccumulated=accumulated+amount,depreciationNo=await nextWorkflowNumber(conn,'accounting_fixed_asset_depreciations','depreciation_no','DEP',lastDate);const[r]=await conn.query(`INSERT INTO accounting_fixed_asset_depreciations(asset_id,tenant_id,company_id,source_system,source_database,depreciation_no,depreciation_period,depreciation_date,depreciation_amount,accumulated_depreciation,status,note,created_by) VALUES(?,?,?,?,?,?,?,?,?,?,'draft',?,?)`,[assetId,...contextParams(c),depreciationNo,period,lastDate,amount,newAccumulated,optionalText(req.body?.note),req.auth.id]);const draftNo=await nextWorkflowNumber(conn,'accounting_drafts','draft_no','FAD',lastDate);const[draft]=await conn.query(`INSERT INTO accounting_drafts(tenant_id,company_id,source_system,source_database,draft_no,draft_date,source_kind,source_id,source_document_no,account_type,party_code,debit_account_code,debit_account_name,credit_account_code,credit_account_name,amount,status,source_locked,source_locked_at,source_locked_by,memo,created_by) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?, 'draft',1,NOW(),?,?,?)`,[...contextParams(c),draftNo,lastDate,'fixed_asset_depreciation',r.insertId,depreciationNo,'FA',null,asset.depreciation_expense_account_code,asset.depreciation_expense_account_name,asset.account_code,asset.account_name,amount,req.auth.id,'G04 折舊自動產生底稿',req.auth.id]);await conn.query(`INSERT INTO accounting_draft_lines(draft_id,line_no,account_code,account_name,debit_amount,credit_amount,party_code,description) VALUES(?,?,?,?,?,?,?,?),(?,?,?,?,?,?,?,?)`,[draft.insertId,1,asset.depreciation_expense_account_code,asset.depreciation_expense_account_name,amount,0,null,`固定資產 ${asset.asset_no} ${period} 折舊`,draft.insertId,2,asset.account_code,asset.account_name,0,amount,null,`固定資產 ${asset.asset_no} ${period} 累計折舊`]);await conn.query(`INSERT INTO accounting_draft_sources(draft_id,source_kind,source_id,source_document_no,source_amount,locked_amount) VALUES(?,?,?,?,?,?)`,[draft.insertId,'fixed_asset_depreciation',r.insertId,depreciationNo,amount,amount]);await conn.query(`INSERT INTO accounting_draft_events(draft_id,event_kind,before_status,after_status,reason,user_id) VALUES(?,?,?,?,?,?)`,[draft.insertId,'create',null,'draft','G04 產生折舊底稿',req.auth.id]);await conn.query('UPDATE accounting_fixed_asset_depreciations SET journal_draft_id=? WHERE id=?',[draft.insertId,r.insertId]);return{id:r.insertId,depreciation_no:depreciationNo,depreciation_period:period,depreciation_amount:amount,accumulated_depreciation:newAccumulated,draft_id:draft.insertId,draft_no:draftNo,status:'draft'};});res.status(201).json({ok:true,data:out});}catch(e){if(e.code==='ER_DUP_ENTRY')e=badRequest('此固定資產與期間的折舊已存在');next(e);}});
  app.post('/api/accounting/g04/depreciation/:id/approve',async(req,res,next)=>{try{await ensureTargetFinanceWorkflowSchema();const c=contextOf(req),id=Number(req.params.id);const out=await tx(async conn=>{const[[dep]]=await conn.query(`SELECT d.*,x.status draft_status FROM accounting_fixed_asset_depreciations d JOIN accounting_drafts x ON x.id=d.journal_draft_id WHERE d.id=? AND d.tenant_id=? AND d.company_id=? AND d.source_system=? AND d.source_database=? AND d.status='draft' FOR UPDATE`,[id,...contextParams(c)]);if(!dep||dep.draft_status!=='draft')throw badRequest('只有折舊底稿草稿可以核准');await assertOpenAccountingPeriod(conn,c,dep.depreciation_date);await conn.query("UPDATE accounting_fixed_asset_depreciations SET status='approved',approved_by=?,approved_at=NOW() WHERE id=?",[req.auth.id,id]);await conn.query("UPDATE accounting_drafts SET status='approved',approved_by=?,approved_at=NOW() WHERE id=?",[req.auth.id,dep.journal_draft_id]);await conn.query(`INSERT INTO accounting_draft_events(draft_id,event_kind,before_status,after_status,reason,user_id) VALUES(?,?,?,?,?,?)`,[dep.journal_draft_id,'approve','draft','approved','核准 G04 折舊底稿',req.auth.id]);return{id,status:'approved',draft_id:dep.journal_draft_id};});res.json({ok:true,data:out});}catch(e){next(e);}});
  app.post('/api/accounting/g04/depreciation/:id/post',async(req,res,next)=>{try{await ensureTargetFinanceWorkflowSchema();const c=contextOf(req),id=Number(req.params.id);const out=await tx(async conn=>{const[[dep]]=await conn.query(`SELECT d.*,x.draft_no,x.status draft_status,x.debit_account_code,x.debit_account_name,x.credit_account_code,x.credit_account_name,x.amount draft_amount,a.asset_no,a.asset_name,a.accumulated_depreciation asset_accumulated,a.net_book_value,a.original_cost,a.residual_value FROM accounting_fixed_asset_depreciations d JOIN accounting_drafts x ON x.id=d.journal_draft_id JOIN accounting_fixed_assets a ON a.id=d.asset_id WHERE d.id=? AND d.tenant_id=? AND d.company_id=? AND d.source_system=? AND d.source_database=? FOR UPDATE`,[id,...contextParams(c)]);if(!dep||dep.status!=='approved'||dep.draft_status!=='approved')throw badRequest('折舊與底稿都核准後才能過帳');await assertOpenAccountingPeriod(conn,c,dep.depreciation_date);const[[already]]=await conn.query("SELECT id,journal_no FROM accounting_journals WHERE tenant_id=? AND company_id=? AND source_system=? AND source_database=? AND source_kind='fixed_asset_depreciation' AND source_id=? AND status<>'voided'",[...contextParams(c),id]);if(already)return{id,journal_id:already.id,journal_no:already.journal_no,status:'posted'};const[j]=await conn.query(`INSERT INTO accounting_journals(tenant_id,company_id,source_system,source_database,journal_no,journal_date,source_kind,source_id,source_document_no,status,memo,created_by,posted_by,posted_at) VALUES(?,?,?,?,?,?,?,?,?,'posted',?,?,?,NOW())`,[...contextParams(c),await nextWorkflowNumber(conn,'accounting_journals','journal_no','FADJ',dep.depreciation_date),dep.depreciation_date,'fixed_asset_depreciation',id,dep.depreciation_no,`固定資產 ${dep.asset_no} 折舊`,req.auth.id,req.auth.id]);const[lines]=await conn.query('SELECT * FROM accounting_draft_lines WHERE draft_id=? ORDER BY line_no',[dep.journal_draft_id]);const debit=lines.reduce((s,x)=>s+Number(x.debit_amount||0),0),credit=lines.reduce((s,x)=>s+Number(x.credit_amount||0),0);if(!lines.length||Math.abs(debit-credit)>0.000001)throw badRequest('折舊底稿借貸不平衡');for(const line of lines)await conn.query(`INSERT INTO accounting_journal_lines(journal_id,line_no,account_code,account_name,debit_amount,credit_amount,party_code,description) VALUES(?,?,?,?,?,?,?,?)`,[j.insertId,line.line_no,line.account_code,line.account_name,line.debit_amount,line.credit_amount,line.party_code,line.description]);await conn.query("UPDATE accounting_drafts SET status='posted',posted_by=?,posted_at=NOW() WHERE id=?",[req.auth.id,dep.journal_draft_id]);await conn.query(`INSERT INTO accounting_draft_events(draft_id,event_kind,before_status,after_status,reason,user_id) VALUES(?,?,?,?,?,?)`,[dep.journal_draft_id,'post','approved','posted','G04 折舊拋轉正式傳票',req.auth.id]);const newAccumulated=Number(dep.asset_accumulated||0)+Number(dep.depreciation_amount),newNet=Math.max(Number(dep.original_cost)-newAccumulated,Number(dep.residual_value||0));await conn.query("UPDATE accounting_fixed_asset_depreciations SET status='posted',posted_by=?,posted_at=NOW() WHERE id=?",[req.auth.id,id]);await conn.query('UPDATE accounting_fixed_assets SET accumulated_depreciation=?,net_book_value=? WHERE id=?',[newAccumulated,newNet,dep.asset_id]);const[[assetAfter]]=await conn.query('SELECT * FROM accounting_fixed_assets WHERE id=?',[dep.asset_id]);await assetEvent(conn,c,dep.asset_id,'depreciation_post',null,assetAfter,`G04 折舊 ${dep.depreciation_no} 過帳`,req.auth.id);return{id,journal_id:j.insertId,journal_no:j.journal_no,status:'posted',accumulated_depreciation:newAccumulated,net_book_value:newNet};});res.json({ok:true,data:out});}catch(e){next(e);}});

  // G04-R1：沿用既有固定資產卡片／事件／折舊／底稿結構，補生命週期稽核與非破壞性異動提案。
  // 處分／減損正式分錄需要原始資產原值科目、往來科目與損益科目等欄位；未取得完整既有結構前，
  // 只建立待核准事件，不變更資產帳面、不產生假分錄。
  const g04JsonValue=value=>{if(value==null)return null;if(typeof value==='object')return value;try{return JSON.parse(String(value));}catch{return null;}};
  const g04DateKey=value=>value instanceof Date?value.toISOString().slice(0,10):String(value||'').slice(0,10);
  async function g04AssetEvents(conn,c,assetId){const[rows]=await conn.query(`SELECT id,event_kind,before_json,after_json,reason,user_id,created_at FROM accounting_fixed_asset_events WHERE asset_id=? AND tenant_id=? AND company_id=? AND source_system=? AND source_database=? ORDER BY id`,[assetId,...contextParams(c)]);return rows.map(row=>({...row,before_json:g04JsonValue(row.before_json),after_json:g04JsonValue(row.after_json)}));}
  async function g04AssetForUpdate(conn,c,id){const[[asset]]=await conn.query(`SELECT * FROM accounting_fixed_assets WHERE id=? AND tenant_id=? AND company_id=? AND source_system=? AND source_database=? FOR UPDATE`,[id,...contextParams(c)]);if(!asset)throw notFound('找不到目前公司／資料來源的固定資產');return asset;}
  function g04PendingEvent(events,kind){return events.find(row=>row.event_kind===kind&&row.after_json?.workflow_status==='pending_account_mapping');}
  function g04LifecycleStatus(asset,events){if(asset.status==='draft')return'draft';if(asset.status==='disposed')return'disposed';if(g04PendingEvent(events,'disposal_proposed'))return'disposal_pending_account_mapping';if(g04PendingEvent(events,'impairment_proposed'))return'impairment_pending_account_mapping';return'active';}
  function g04Reason(body,label){const reason=trim(body?.reason);if(!reason)throw badRequest(`${label}必須填寫原因`);if(reason.length>500)throw badRequest(`${label}原因不可超過 500 字`);return reason;}
  async function g04OpenEventDate(conn,c,value,label){const date=validDate(value||new Date().toISOString().slice(0,10));await assertOpenAccountingPeriod(conn,c,date);return date;}
  app.get('/api/accounting/g04/rules',async(req,res,next)=>{try{await ensureTargetFinanceWorkflowSchema();const c=contextOf(req);res.json({ok:true,data:{source_database:c.source_database,company_id:c.company_id,supported_lifecycle:['acquisition_card','transfer'],proposal_lifecycle:['disposal_proposed','impairment_proposed'],supported_depreciation_methods:['straight_line'],unsupported_depreciation_methods:['declining_balance','sum_of_years_digits','units_of_production'],period_control:'assertOpenAccountingPeriod',posting_control:'approved_asset→draft→approve→post',source_tables:['accounting_fixed_assets','accounting_fixed_asset_events','accounting_fixed_asset_depreciations','accounting_drafts','accounting_journals'],warnings:['處分／減損正式分錄仍需原始資產原值科目、往來科目、損益科目與核准欄位對照，不在本階段猜測或新增結構']}});}catch(e){next(e);}});
  app.get('/api/accounting/g04/report',async(req,res,next)=>{try{await ensureTargetFinanceWorkflowSchema();const c=contextOf(req),from=validDate(req.query.date_from||'1900-01-01'),to=validDate(req.query.date_to||new Date().toISOString().slice(0,10));if(from>to)throw badRequest('固定資產報表日期起日不可晚於迄日');const where=['a.tenant_id=?','a.company_id=?','a.source_system=?','a.source_database=?'],params=[...contextParams(c)];if(trim(req.query.asset_no)){where.push('a.asset_no=?');params.push(trim(req.query.asset_no));}if(trim(req.query.department_code)){where.push('COALESCE(a.department_code,\'\')=?');params.push(trim(req.query.department_code));}if(trim(req.query.category_code)){where.push('COALESCE(a.category_code,\'\')=?');params.push(trim(req.query.category_code));}if(trim(req.query.status)){where.push('a.status=?');params.push(trim(req.query.status));}const[assets]=await pool.query(`SELECT a.* FROM accounting_fixed_assets a WHERE ${where.join(' AND ')} ORDER BY a.asset_no`,params),assetIds=assets.map(row=>Number(row.id));let deps=[],events=[];if(assetIds.length){const[d]=await pool.query(`SELECT * FROM accounting_fixed_asset_depreciations WHERE tenant_id=? AND company_id=? AND source_system=? AND source_database=? AND asset_id IN (?) ORDER BY asset_id,depreciation_period`,[...contextParams(c),assetIds]);deps=d.filter(row=>g04DateKey(row.depreciation_date)>=from&&g04DateKey(row.depreciation_date)<=to);const[e]=await pool.query(`SELECT id,asset_id,event_kind,after_json,reason,user_id,created_at FROM accounting_fixed_asset_events WHERE tenant_id=? AND company_id=? AND source_system=? AND source_database=? AND asset_id IN (?) ORDER BY asset_id,id`,[...contextParams(c),assetIds]);events=e.map(row=>({...row,after_json:g04JsonValue(row.after_json)}));}const accountCodes=[...new Set(assets.flatMap(row=>[row.account_code,row.depreciation_expense_account_code]).map(value=>trim(value)).filter(Boolean))],accountMap=new Map();if(accountCodes.length){const[accountRows]=await pool.query(`SELECT account_code,account_name,status,is_active FROM accounting_accounts WHERE tenant_id=? AND company_id=? AND source_system=? AND account_code IN (?)`,[c.tenant_id,c.company_id,c.source_system,accountCodes]);for(const row of accountRows)accountMap.set(String(row.account_code),row);}const depByAsset=new Map(),eventByAsset=new Map();for(const row of deps){const list=depByAsset.get(Number(row.asset_id))||[];list.push(row);depByAsset.set(Number(row.asset_id),list);}for(const row of events){const list=eventByAsset.get(Number(row.asset_id))||[];list.push(row);eventByAsset.set(Number(row.asset_id),list);}const warnings=[],rows=assets.map(asset=>{const assetEvents=eventByAsset.get(Number(asset.id))||[],assetDeps=depByAsset.get(Number(asset.id))||[],posted=assetDeps.filter(row=>row.status==='posted'),pending=assetDeps.filter(row=>['draft','approved'].includes(String(row.status))),account=accountMap.get(String(asset.account_code)),expense=accountMap.get(String(asset.depreciation_expense_account_code)),lifecycle=g04LifecycleStatus(asset,assetEvents),mappingStatus=account&&expense&&Number(account.is_active)!==0&&Number(expense.is_active)!==0&&account.status==='approved'&&expense.status==='approved'?'ready':'account_mapping_warning';if(mappingStatus!=='ready')warnings.push({code:'ACCOUNT_MAPPING',severity:'warning',asset_no:asset.asset_no,message:`${asset.asset_no} 的資產／折舊科目不存在、停用或尚未核准`});if(String(asset.depreciation_method)!=='straight_line')warnings.push({code:'DEPRECIATION_METHOD',severity:'warning',asset_no:asset.asset_no,message:`${asset.asset_no} 使用尚未支援的折舊方法：${asset.depreciation_method}`});if(lifecycle.endsWith('account_mapping'))warnings.push({code:'LIFECYCLE_PROPOSAL',severity:'warning',asset_no:asset.asset_no,message:`${asset.asset_no} 有待補科目映射的${lifecycle.startsWith('disposal')?'處分':'減損'}提案，尚未變更帳面或產生正式分錄`});return{...asset,lifecycle_status:lifecycle,mapping_status:mappingStatus,posted_depreciation_amount:posted.reduce((sum,row)=>sum+Number(row.depreciation_amount||0),0),pending_depreciation_amount:pending.reduce((sum,row)=>sum+Number(row.depreciation_amount||0),0),posted_depreciation_count:posted.length,pending_depreciation_count:pending.length,event_count:assetEvents.length,last_event:assetEvents.at(-1)||null,events:assetEvents.map(row=>({id:row.id,event_kind:row.event_kind,reason:row.reason,user_id:row.user_id,created_at:row.created_at,workflow_status:row.after_json?.workflow_status||null}))};});const summary={asset_count:rows.length,original_cost:rows.reduce((sum,row)=>sum+Number(row.original_cost||0),0),accumulated_depreciation:rows.reduce((sum,row)=>sum+Number(row.accumulated_depreciation||0),0),net_book_value:rows.reduce((sum,row)=>sum+Number(row.net_book_value||0),0),posted_depreciation_amount:rows.reduce((sum,row)=>sum+Number(row.posted_depreciation_amount||0),0),pending_depreciation_count:rows.reduce((sum,row)=>sum+Number(row.pending_depreciation_count||0),0),proposal_count:rows.reduce((sum,row)=>sum+row.events.filter(event=>['disposal_proposed','impairment_proposed'].includes(event.event_kind)&&event.workflow_status==='pending_account_mapping').length,0),warning_count:warnings.length};res.json({ok:true,data:{source_database:c.source_database,company_id:c.company_id,date_from:from,date_to:to,filters:{asset_no:trim(req.query.asset_no)||null,department_code:trim(req.query.department_code)||null,category_code:trim(req.query.category_code)||null,status:trim(req.query.status)||null},supported_depreciation_methods:['straight_line'],rows,summary,warnings}});}catch(e){next(e);}});
  app.post('/api/accounting/g04/assets/:id/transfer',async(req,res,next)=>{try{await ensureTargetFinanceWorkflowSchema();const c=contextOf(req),id=integer(req.params.id,'固定資產',{min:1}),reason=g04Reason(req.body,'移轉'),targetDepartment=optionalText(req.body?.target_department_code),targetLocation=optionalText(req.body?.target_location);const out=await tx(async conn=>{const asset=await g04AssetForUpdate(conn,c,id);if(asset.status!=='approved')throw badRequest('只有已核准且未處分的固定資產可以移轉');const date=await g04OpenEventDate(conn,c,req.body?.effective_date,'移轉日期'),events=await g04AssetEvents(conn,c,id);if(g04PendingEvent(events,'disposal_proposed'))throw badRequest('固定資產已有待補科目映射的處分提案，完成受控處理前不可移轉');if(String(asset.department_code||'')===String(targetDepartment||'')&&String(asset.location||'')===String(targetLocation||''))throw badRequest('移轉後部門與存放地點不可與原值完全相同');await conn.query('UPDATE accounting_fixed_assets SET department_code=?,location=? WHERE id=?',[targetDepartment,targetLocation,id]);const[[after]]=await conn.query('SELECT * FROM accounting_fixed_assets WHERE id=?',[id]);await assetEvent(conn,c,id,'transfer',asset,after,`G04 資產移轉 ${date}；${reason}`,req.auth.id);return{asset_id:id,asset_no:after.asset_no,effective_date:date,status:after.status,department_code:after.department_code,location:after.location};});res.json({ok:true,data:out});}catch(e){next(e);}});
  app.post('/api/accounting/g04/assets/:id/disposal',async(req,res,next)=>{try{await ensureTargetFinanceWorkflowSchema();const c=contextOf(req),id=integer(req.params.id,'固定資產',{min:1}),reason=g04Reason(req.body,'處分'),proceeds=decimal(req.body?.proceeds_amount??0,'處分收入',{min:0}),assetAccount=optionalText(req.body?.asset_account_code),settlementAccount=optionalText(req.body?.settlement_account_code),gainLossAccount=optionalText(req.body?.gain_loss_account_code);const out=await tx(async conn=>{const asset=await g04AssetForUpdate(conn,c,id);if(asset.status!=='approved')throw badRequest('只有已核准且未處分的固定資產可以建立處分提案');const date=await g04OpenEventDate(conn,c,req.body?.disposal_date,'處分日期'),events=await g04AssetEvents(conn,c,id);if(g04PendingEvent(events,'disposal_proposed'))throw badRequest('此固定資產已有待處分提案');if(g04PendingEvent(events,'impairment_proposed'))throw badRequest('此固定資產已有待減損提案，完成受控處理前不可建立處分提案');const proposal={workflow_status:'pending_account_mapping',financial_posting:false,disposal_date:date,proceeds_amount:proceeds,asset_account_code:assetAccount,settlement_account_code:settlementAccount,gain_loss_account_code:gainLossAccount,required_account_fields:['asset_account_code','settlement_account_code','gain_loss_account_code'],original_cost:Number(asset.original_cost||0),accumulated_depreciation:Number(asset.accumulated_depreciation||0),net_book_value:Number(asset.net_book_value||0),note:optionalText(req.body?.note)};await assetEvent(conn,c,id,'disposal_proposed',asset,proposal,reason,req.auth.id);return{asset_id:id,asset_no:asset.asset_no,status:'pending_account_mapping',proposal};});res.status(201).json({ok:true,data:out});}catch(e){next(e);}});
  app.post('/api/accounting/g04/assets/:id/impairment',async(req,res,next)=>{try{await ensureTargetFinanceWorkflowSchema();const c=contextOf(req),id=integer(req.params.id,'固定資產',{min:1}),reason=g04Reason(req.body,'減損'),amount=decimal(req.body?.impairment_amount,'減損金額',{min:0.000001}),lossAccount=optionalText(req.body?.impairment_loss_account_code),contraAccount=optionalText(req.body?.impairment_contra_account_code);const out=await tx(async conn=>{const asset=await g04AssetForUpdate(conn,c,id);if(asset.status!=='approved')throw badRequest('只有已核准且未處分的固定資產可以建立減損提案');const date=await g04OpenEventDate(conn,c,req.body?.impairment_date,'減損日期'),events=await g04AssetEvents(conn,c,id);if(g04PendingEvent(events,'impairment_proposed'))throw badRequest('此固定資產已有待減損提案');if(g04PendingEvent(events,'disposal_proposed'))throw badRequest('此固定資產已有待處分提案，完成受控處理前不可建立減損提案');if(amount>Number(asset.net_book_value||0)+0.000001)throw badRequest('減損金額不可大於目前帳面淨額');const proposal={workflow_status:'pending_account_mapping',financial_posting:false,impairment_date:date,impairment_amount:amount,impairment_loss_account_code:lossAccount,impairment_contra_account_code:contraAccount,required_account_fields:['impairment_loss_account_code','impairment_contra_account_code'],net_book_value_before:Number(asset.net_book_value||0),note:optionalText(req.body?.note)};await assetEvent(conn,c,id,'impairment_proposed',asset,proposal,reason,req.auth.id);return{asset_id:id,asset_no:asset.asset_no,status:'pending_account_mapping',proposal};});res.status(201).json({ok:true,data:out});}catch(e){next(e);}});

  function centerPayload(body){const code=trim(body.center_code),name=trim(body.center_name);if(!code||!name)throw badRequest('利潤中心代號與名稱不可空白');return{code,name,departmentCode:optionalText(body.department_code),parentCode:optionalText(body.parent_center_code),basis:trim(body.allocation_basis||'direct'),note:optionalText(body.note)};}
  function allocationPayload(body){const code=trim(body.allocation_code),name=trim(body.allocation_name),year=trim(body.fiscal_year),account=trim(body.source_account_code),center=trim(body.target_center_code),ratio=decimal(body.allocation_ratio,'分攤比率',{min:0,max:1});if(!code||!name||!/^\d{4}$/.test(year)||!account||!center)throw badRequest('分攤代號、名稱、年度、來源科目與目標利潤中心不可空白');return{code,name,year,account,center,departmentCode:optionalText(body.target_department_code),ratio,note:optionalText(body.note)};}
  // G05 利潤中心管理：中心、分攤比率、核准與已過帳金額分攤預覽。
  app.get('/api/accounting/g05/centers',async(req,res,next)=>{try{await ensureTargetFinanceWorkflowSchema();const c=contextOf(req),[rows]=await pool.query(`SELECT * FROM accounting_profit_centers WHERE tenant_id=? AND company_id=? AND source_system=? AND source_database=? ORDER BY center_code`,contextParams(c));res.json({ok:true,data:rows,source_database:c.source_database});}catch(e){next(e);}});
  app.post('/api/accounting/g05/centers',async(req,res,next)=>{try{await ensureTargetFinanceWorkflowSchema();const c=contextOf(req),p=centerPayload(req.body||{});const[r]=await pool.query(`INSERT INTO accounting_profit_centers(tenant_id,company_id,source_system,source_database,center_code,center_name,department_code,parent_center_code,allocation_basis,status,is_active,note,created_by) VALUES(?,?,?,?,?,?,?,?,?,'draft',1,?,?)`,[...contextParams(c),p.code,p.name,p.departmentCode,p.parentCode,p.basis,p.note,req.auth.id]);const[[row]]=await pool.query('SELECT * FROM accounting_profit_centers WHERE id=?',[r.insertId]);await pool.query(`INSERT INTO accounting_profit_center_events(entity_kind,entity_id,tenant_id,company_id,source_system,source_database,event_kind,after_json,reason,user_id) VALUES(?,?,?,?,?,?,?,?,?,?)`,['center',r.insertId,...contextParams(c),'create',json(row),'建立 G05 利潤中心草稿',req.auth.id]);res.status(201).json({ok:true,data:row});}catch(e){if(e.code==='ER_DUP_ENTRY')e=badRequest('利潤中心代號已存在');next(e);}});
  app.put('/api/accounting/g05/centers/:id',async(req,res,next)=>{try{await ensureTargetFinanceWorkflowSchema();const c=contextOf(req),id=Number(req.params.id),p=centerPayload(req.body||{});const out=await tx(async conn=>{const[[before]]=await conn.query(`SELECT * FROM accounting_profit_centers WHERE id=? AND tenant_id=? AND company_id=? AND source_system=? AND source_database=? FOR UPDATE`,[id,...contextParams(c)]);if(!before)throw notFound('找不到目前公司的利潤中心');await conn.query(`UPDATE accounting_profit_centers SET center_name=?,department_code=?,parent_center_code=?,allocation_basis=?,status='draft',approved_by=NULL,approved_at=NULL,note=? WHERE id=?`,[p.name,p.departmentCode,p.parentCode,p.basis,p.note,id]);const[[after]]=await conn.query('SELECT * FROM accounting_profit_centers WHERE id=?',[id]);await profitEvent(conn,c,'center',id,'update',before,after,'修改 G05 利潤中心',req.auth.id);return after;});res.json({ok:true,data:out});}catch(e){next(e);}});
  app.post('/api/accounting/g05/centers/:id/approve',async(req,res,next)=>{try{await ensureTargetFinanceWorkflowSchema();const c=contextOf(req),id=Number(req.params.id);const out=await tx(async conn=>{const[[before]]=await conn.query(`SELECT * FROM accounting_profit_centers WHERE id=? AND tenant_id=? AND company_id=? AND source_system=? AND source_database=? AND status='draft' FOR UPDATE`,[id,...contextParams(c)]);if(!before)throw badRequest('只有 G05 草稿利潤中心可以核准');if(before.parent_center_code){const[[parent]]=await conn.query(`SELECT id FROM accounting_profit_centers WHERE center_code=? AND tenant_id=? AND company_id=? AND source_system=? AND source_database=? AND status='approved'`,[before.parent_center_code,...contextParams(c)]);if(!parent)throw badRequest('上層利潤中心不存在或尚未核准');}await conn.query("UPDATE accounting_profit_centers SET status='approved',is_active=1,approved_by=?,approved_at=NOW() WHERE id=?",[req.auth.id,id]);const[[after]]=await conn.query('SELECT * FROM accounting_profit_centers WHERE id=?',[id]);await profitEvent(conn,c,'center',id,'approve',before,after,'核准 G05 利潤中心',req.auth.id);return after;});res.json({ok:true,data:out});}catch(e){next(e);}});
  app.get('/api/accounting/g05/allocations',async(req,res,next)=>{try{await ensureTargetFinanceWorkflowSchema();const c=contextOf(req),[rows]=await pool.query(`SELECT a.*,p.center_name FROM accounting_profit_center_allocations a LEFT JOIN accounting_profit_centers p ON p.center_code=a.target_center_code AND p.tenant_id=a.tenant_id AND p.company_id=a.company_id AND p.source_system=a.source_system AND p.source_database=a.source_database WHERE a.tenant_id=? AND a.company_id=? AND a.source_system=? AND a.source_database=? ORDER BY a.fiscal_year,a.allocation_code,a.source_account_code,a.target_center_code`,contextParams(c));res.json({ok:true,data:rows,source_database:c.source_database});}catch(e){next(e);}});
  app.post('/api/accounting/g05/allocations',async(req,res,next)=>{try{await ensureTargetFinanceWorkflowSchema();const c=contextOf(req),p=allocationPayload(req.body||{});const out=await tx(async conn=>{await requireAccounts(conn,c,[p.account],'分攤來源科目');const[[center]]=await conn.query(`SELECT id FROM accounting_profit_centers WHERE center_code=? AND tenant_id=? AND company_id=? AND source_system=? AND source_database=? AND status='approved' AND is_active=1`,[p.center,...contextParams(c)]);if(!center)throw badRequest('目標利潤中心不存在或尚未核准');const[r]=await conn.query(`INSERT INTO accounting_profit_center_allocations(tenant_id,company_id,source_system,source_database,allocation_code,allocation_name,fiscal_year,source_account_code,target_center_code,target_department_code,allocation_ratio,status,note,created_by) VALUES(?,?,?,?,?,?,?,?,?,?,?,'draft',?,?)`,[...contextParams(c),p.code,p.name,p.year,p.account,p.center,p.departmentCode,p.ratio,p.note,req.auth.id]);const[[row]]=await conn.query('SELECT * FROM accounting_profit_center_allocations WHERE id=?',[r.insertId]);await profitEvent(conn,c,'allocation',r.insertId,'create',null,row,'建立 G05 分攤比率草稿',req.auth.id);return row;});res.status(201).json({ok:true,data:out});}catch(e){if(e.code==='ER_DUP_ENTRY')e=badRequest('同一分攤代號、來源科目與目標中心已存在');next(e);}});
  app.put('/api/accounting/g05/allocations/:id',async(req,res,next)=>{try{await ensureTargetFinanceWorkflowSchema();const c=contextOf(req),id=Number(req.params.id),p=allocationPayload(req.body||{});const out=await tx(async conn=>{const[[before]]=await conn.query(`SELECT * FROM accounting_profit_center_allocations WHERE id=? AND tenant_id=? AND company_id=? AND source_system=? AND source_database=? FOR UPDATE`,[id,...contextParams(c)]);if(!before)throw notFound('找不到目前公司的分攤比率');await requireAccounts(conn,c,[p.account],'分攤來源科目');const[[center]]=await conn.query(`SELECT id FROM accounting_profit_centers WHERE center_code=? AND tenant_id=? AND company_id=? AND source_system=? AND source_database=? AND status='approved' AND is_active=1`,[p.center,...contextParams(c)]);if(!center)throw badRequest('目標利潤中心不存在或尚未核准');await conn.query(`UPDATE accounting_profit_center_allocations SET allocation_name=?,fiscal_year=?,source_account_code=?,target_center_code=?,target_department_code=?,allocation_ratio=?,status='draft',approved_by=NULL,approved_at=NULL,note=? WHERE id=?`,[p.name,p.year,p.account,p.center,p.departmentCode,p.ratio,p.note,id]);const[[after]]=await conn.query('SELECT * FROM accounting_profit_center_allocations WHERE id=?',[id]);await profitEvent(conn,c,'allocation',id,'update',before,after,'修改 G05 分攤比率',req.auth.id);return after;});res.json({ok:true,data:out});}catch(e){next(e);}});
  app.post('/api/accounting/g05/allocations/:id/approve',async(req,res,next)=>{try{await ensureTargetFinanceWorkflowSchema();const c=contextOf(req),id=Number(req.params.id);const out=await tx(async conn=>{const[[before]]=await conn.query(`SELECT * FROM accounting_profit_center_allocations WHERE id=? AND tenant_id=? AND company_id=? AND source_system=? AND source_database=? AND status='draft' FOR UPDATE`,[id,...contextParams(c)]);if(!before)throw badRequest('只有 G05 草稿分攤比率可以核准');const[[center]]=await conn.query(`SELECT id FROM accounting_profit_centers WHERE center_code=? AND tenant_id=? AND company_id=? AND source_system=? AND source_database=? AND status='approved' AND is_active=1`,[before.target_center_code,...contextParams(c)]);if(!center)throw badRequest('目標利潤中心不存在或尚未核准');const[[sum]]=await conn.query(`SELECT COALESCE(SUM(allocation_ratio),0) total_ratio FROM accounting_profit_center_allocations WHERE tenant_id=? AND company_id=? AND source_system=? AND source_database=? AND allocation_code=? AND fiscal_year=? AND source_account_code=? AND status IN ('draft','approved')`,[...contextParams(c),before.allocation_code,before.fiscal_year,before.source_account_code]);if(Math.abs(Number(sum.total_ratio)-1)>0.000001)throw badRequest(`分攤比率合計必須等於 1，目前為 ${Number(sum.total_ratio)}`);await conn.query("UPDATE accounting_profit_center_allocations SET status='approved',approved_by=?,approved_at=NOW() WHERE id=?",[req.auth.id,id]);const[[after]]=await conn.query('SELECT * FROM accounting_profit_center_allocations WHERE id=?',[id]);await profitEvent(conn,c,'allocation',id,'approve',before,after,'核准 G05 分攤比率',req.auth.id);return after;});res.json({ok:true,data:out});}catch(e){next(e);}});
  app.get('/api/accounting/g05/report',async(req,res,next)=>{try{await ensureTargetFinanceWorkflowSchema();const c=contextOf(req),year=trim(req.query.fiscal_year);if(!/^\d{4}$/.test(year))throw badRequest('利潤中心報表必須指定四碼年度');const[centers]=await pool.query(`SELECT center_code,center_name,department_code FROM accounting_profit_centers WHERE tenant_id=? AND company_id=? AND source_system=? AND source_database=? AND status='approved' AND is_active=1`,contextParams(c));const[allocations]=await pool.query(`SELECT allocation_code,fiscal_year,source_account_code,target_center_code,target_department_code,allocation_ratio FROM accounting_profit_center_allocations WHERE tenant_id=? AND company_id=? AND source_system=? AND source_database=? AND fiscal_year=? AND status='approved'`,[...contextParams(c),year]);const[actuals]=await pool.query(`SELECT l.account_code,l.account_name,l.department_code,l.profit_center_code,a.normal_balance,SUM(CASE WHEN COALESCE(a.normal_balance,'debit')='credit' THEN l.credit_amount-l.debit_amount ELSE l.debit_amount-l.credit_amount END) amount FROM accounting_journals j JOIN accounting_journal_lines l ON l.journal_id=j.id LEFT JOIN accounting_accounts a ON a.tenant_id=j.tenant_id AND a.company_id=j.company_id AND a.source_system=j.source_system AND a.account_code=l.account_code WHERE j.tenant_id=? AND j.company_id=? AND j.source_system=? AND j.source_database=? AND j.status='posted' AND YEAR(j.journal_date)=? GROUP BY l.account_code,l.account_name,l.department_code,l.profit_center_code,a.normal_balance`,[...contextParams(c),year]);const map=new Map(),add=(center,account,name,amount,source='direct')=>{const key=`${center||'UNALLOCATED'}|${account}`;const row=map.get(key)||{profit_center_code:center||null,account_code:account,account_name:name,amount:0,source};row.amount+=Number(amount||0);map.set(key,row);};const allocMap=new Map();for(const row of allocations){const key=`${row.source_account_code}`;const list=allocMap.get(key)||[];list.push(row);allocMap.set(key,list);}for(const row of actuals){if(row.profit_center_code){add(row.profit_center_code,row.account_code,row.account_name,row.amount,'journal');continue;}const list=allocMap.get(String(row.account_code))||[];if(!list.length){add(null,row.account_code,row.account_name,row.amount,'unallocated');continue;}for(const allocation of list)add(allocation.target_center_code,row.account_code,row.account_name,Number(row.amount)*Number(allocation.allocation_ratio),'allocated');}const centerTotals={};for(const row of map.values())centerTotals[row.profit_center_code||'UNALLOCATED']=(centerTotals[row.profit_center_code||'UNALLOCATED']||0)+row.amount;res.json({ok:true,data:{fiscal_year:year,centers,allocations,rows:[...map.values()],center_totals:centerTotals,actual_rows:actuals}});}catch(e){next(e);}});
}
