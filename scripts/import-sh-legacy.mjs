...
const source = String(process.argv[2] || 'SH').toUpperCase();
if (!/^[A-Z0-9_]+$/.test(source)) throw new Error('Invalid source database');
const target = process.env.DB_NAME || 'inventory_erp';
...
const S = q(source);
const context = { tenant: source, company: source, system: 'iSM' };
const batchNo = `LEGACY-${source}-${new Date().toISOString().replace(/[-:.TZ]/g, '').slice(0,14)}`;
const [batchResult] = await db.execute(`INSERT INTO erp_import_batches
  ...
  VALUES(?,?,?,?,?,'running',NOW(),8,?)`, [batchNo,context.tenant,context.company,context.system,source,'舊 ERP 單據同步；來源資料庫維持唯讀']);
...
await run('銷售訂單表頭','copta','sales_documents',
  `SELECT COUNT(*) n FROM ${S}.copta WHERE COMPANY=${db.escape(source)} AND ${valid('TA003')}`,
  `INSERT IGNORE INTO sales_documents(tenant_id,company_id,source_system,source_database,document_kind,document_type,document_no,document_date,customer_code,currency_code,salesperson_code,status,inventory_status,note)
   SELECT ${db.escape(context.tenant)},${db.escape(context.company)},${db.escape(context.system)},${db.escape(source)},'sales_order',TA001,CONCAT(TA001,'/',TA002),STR_TO_DATE(TA003,'%Y%m%d'),TA004,IF(TA007 IN ('','NTD'),'TWD',TA007),TA005,'completed','not_applicable',CONCAT('舊ERP匯入；原單號 ',TA002)
   FROM ${S}.copta WHERE COMPANY=${db.escape(source)} AND ${valid('TA003')}`);
await run('銷售訂單明細','coptb','sales_document_items',
  `SELECT COUNT(*) n FROM ${S}.coptb b JOIN sales_documents d ON d.tenant_id=${db.escape(context.tenant)} AND d.company_id=${db.escape(context.company)} AND d.source_system=${db.escape(context.system)} AND d.document_kind='sales_order' AND d.document_no=CONCAT(b.TB001,'/',b.TB002) WHERE b.COMPANY=${db.escape(source)}`,
  `INSERT IGNORE INTO sales_document_items(document_id,line_no,item_code,item_name,specification,unit,warehouse_code,quantity,unit_price,expected_date,note)
   ...
   WHERE b.COMPANY=${db.escape(source)}`);

await run('銷貨表頭','coptg','sales_documents',
  `SELECT COUNT(*) n FROM ${S}.coptg WHERE COMPANY=${db.escape(source)} AND ${valid('TG003')}`,
  `INSERT IGNORE INTO sales_documents(tenant_id,company_id,source_system,source_database,document_kind,document_type,document_no,document_date,customer_code,currency_code,salesperson_code,status,inventory_status,note)
   SELECT ${db.escape(context.tenant)},${db.escape(context.company)},${db.escape(context.system)},${db.escape(source)},'shipment',TG001,CONCAT(TG001,'/',TG002),STR_TO_DATE(TG003,'%Y%m%d'),TG004,IF(TG011 IN ('','NTD'),'TWD',TG011),TG006,'posted','posted',CONCAT('舊ERP匯入；原單號 ',TG002)
   FROM ${S}.coptg WHERE COMPANY=${db.escape(source)} AND ${valid('TG003')}`);
await run('銷貨明細','copth','sales_document_items',
  `SELECT COUNT(*) n FROM ${S}.copth h JOIN sales_documents d ON d.tenant_id=${db.escape(context.tenant)} AND d.company_id=${db.escape(context.company)} AND d.source_system=${db.escape(context.system)} AND d.document_kind='shipment' AND d.document_no=CONCAT(h.TH001,'/',h.TH002) WHERE h.COMPANY=${db.escape(source)}`,
  `INSERT IGNORE INTO sales_document_items(document_id,line_no,item_code,item_name,specification,unit,warehouse_code,quantity,unit_price,note)
   ...
   WHERE h.COMPANY=${db.escape(source)}`);

await run('採購單表頭','purtc','procurement_orders',
  `SELECT COUNT(*) n FROM ${S}.purtc WHERE COMPANY=${db.escape(source)} AND ${valid('TC003')}`,
  `INSERT IGNORE INTO procurement_orders(purchase_order_no,supplier_code,order_date,currency_code,status,note,source_database,tenant_id,company_id,source_system,document_type)
   SELECT CONCAT(TC001,'/',TC002),TC004,STR_TO_DATE(TC003,'%Y%m%d'),IF(TC005 IN ('','NTD'),'TWD',TC005),'confirmed',CONCAT('舊ERP匯入；原單號 ',TC002,' ',TC009),${db.escape(source)},${db.escape(context.tenant)},${db.escape(context.company)},${db.escape(context.system)},TC001
   FROM ${S}.purtc WHERE COMPANY=${db.escape(source)} AND ${valid('TC003')}`);
await run('採購單明細','purtd','procurement_order_items',
  `SELECT COUNT(*) n FROM ${S}.purtd d JOIN procurement_orders o ON o.source_database=${db.escape(source)} AND o.purchase_order_no=CONCAT(d.TD001,'/',d.TD002) WHERE d.COMPANY=${db.escape(source)}`,
  `INSERT IGNORE INTO procurement_order_items(purchase_order_id,line_no,item_code,item_name,specification,warehouse_code,unit,qty_ordered,qty_received,unit_price,expected_date,note)
   ...
   FROM ${S}.purtd d JOIN procurement_orders o ON o.source_database=${db.escape(source)} AND o.purchase_order_no=CONCAT(d.TD001,'/',d.TD002)
   WHERE d.COMPANY=${db.escape(source)}`);

await run('進貨表頭','purtg','procurement_receipts',
  `SELECT COUNT(*) n FROM ${S}.purtg WHERE COMPANY=${db.escape(source)} AND ${valid('TG014')}`,
  `INSERT IGNORE INTO procurement_receipts(receipt_no,supplier_code,receipt_date,status,note,source_database,tenant_id,company_id,source_system,document_type,inventory_status)
   SELECT CONCAT(TG001,'/',TG002),TG005,STR_TO_DATE(TG014,'%Y%m%d'),'posted',CONCAT('舊ERP匯入；原單號 ',TG002,' ',TG016),${db.escape(source)},${db.escape(context.tenant)},${db.escape(context.company)},${db.escape(context.system)},TG001,'posted'
   FROM ${S}.purtg WHERE COMPANY=${db.escape(source)} AND ${valid('TG014')}`);
await run('進貨明細','purth','procurement_receipt_items',
  `SELECT COUNT(*) n FROM ${S}.purth d JOIN procurement_receipts r ON r.source_database=${db.escape(source)} AND r.receipt_no=CONCAT(d.TH001,'/',d.TH002) WHERE d.COMPANY=${db.escape(source)}`,
  `INSERT IGNORE INTO procurement_receipt_items(receipt_id,line_no,item_code,item_name,specification,warehouse_code,unit,qty_received,qty_accepted,unit_cost,lot_no,note,inspection_status)
   ...
   FROM ${S}.purth d JOIN procurement_receipts r ON r.source_database=${db.escape(source)} AND r.receipt_no=CONCAT(d.TH001,'/',d.TH002)
   WHERE d.COMPANY=${db.escape(source)}`);

await run('應收立帳','acrta','finance_open_items',
  `SELECT COUNT(*) n FROM ${S}.acrta WHERE COMPANY=${db.escape(source)} AND ${valid('TA003')}`,
...
await run('收款紀錄','acrtc','finance_settlements',
  ...
  `INSERT IGNORE INTO finance_settlements(tenant_id,company_id,source_system,source_database,account_type,settlement_no,settlement_date,party_code,payment_method,amount,status,note)
   SELECT ${db.escape(context.tenant)},${db.escape(context.company)},${db.escape(context.system)},${db.escape(source)},'AR',CONCAT(TC001,'/',TC002),STR_TO_DATE(TC003,'%Y%m%d'),TC004,'legacy',ABS(COALESCE(TC014,0)),'posted',CONCAT('舊ERP匯入；',TC007)
   FROM ${S}.acrtc WHERE COMPANY=${db.escape(source)} AND ${valid('TC003')}`);
await run('付款紀錄','acptc','finance_settlements',
  `SELECT COUNT(*) n FROM ${S}.acptc WHERE COMPANY=${db.escape(source)} AND ${valid('TC003')}`,
  `INSERT IGNORE INTO finance_settlements(tenant_id,company_id,source_system,source_database,account_type,settlement_no,settlement_date,party_code,payment_method,amount,status,note)
   SELECT ${db.escape(context.tenant)},${db.escape(context.company)},${db.escape(context.system)},${db.escape(source)},'AP',CONCAT(TC001,'/',TC002),STR_TO_DATE(TC003,'%Y%m%d'),TC004,'legacy',ABS(COALESCE(TC014,0)),'posted',CONCAT('舊ERP匯入；',TC007)
   FROM ${S}.acptc WHERE COMPANY=${db.escape(source)} AND ${valid('TC003')}`);
